...
<script language="javascript">
	...
	function Check()
	{
		...
		with(theForm){
			...
			if(!field_check(theForm.dateStartSfStr, 'СФ период с', 0, true)) return false;
...
<july:separator colspan="<%= gColSpan %>"/>	
	<tr>
		...
		<td class="text">
			...
			Дата:<july:date property="dateSchetFaktStr" size="20" insertable="true" updatable="true" required="false" styleId="dateSchetFaktStr"/>
			<%--logic:equal name="<%= form %>" property="vo.state" value="O">    	    	
				<INPUT TYPE="IMAGE" NAME="submit" SRC="/images/obchet1.gif"  ALT="Рассчитать дату СФ" onclick="return computeSF('computeDateSF');">
			</logic:equal--%>			
		</td>
    </tr>
<july:separator colspan="<%= gColSpan %>"/>	
	<tr>
		...
		<td class="text">
			с :<july:date property="dateStartSfStr" size="20" insertable="true" updatable="<%=isEdit%>" required="false" styleId="dateStartSfStr"/>
			по:<july:date property="dateEndSfStr" size="20" insertable="true" updatable="<%=isEdit%>" required="false" styleId="dateEndSfStr"/>
			<logic:equal name="<%= form %>" property="vo.state" value="O">    	    	
				<%--INPUT TYPE="IMAGE" NAME="submit" SRC="/images/obchet1.gif"  ALT="Рассчитать период СФ" onclick="return computeSF('computePeriodSF');"--%>
				<INPUT TYPE="IMAGE" NAME="submit" SRC="/images/obchet1.gif"  ALT="Задать дату и период СФ" onclick="return computeSF('computeDateAndPeriodSF');">
			</logic:equal>			
		</td>
    </tr>
<july:separator colspan="<%= gColSpan %>"/>
