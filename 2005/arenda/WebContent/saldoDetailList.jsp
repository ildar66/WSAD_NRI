...
<% 
	String gColSpan = "9";
	SaldoDetailListForm listForm =
	(SaldoDetailListForm) session.getAttribute("SaldoDetailListForm");
	String sortBy = listForm.getSortBy();
%>
<%
    String action = "/SaldoDetailList.do";
	String form = "SaldoDetailListForm";
	String edit = "/SaldoDetailDialog.do";
...
<table class="main" id="tableID" style="font-size: 9pt">
	...
	<TR class="normal">
		<TD colspan="<%= gColSpan %>">
			<table class="find">
				<tr>
					<td class="text">
						<A href='AgreementInfo.do?action=View&leaseDocument=<bean:write name="<%= form %>" property="mainLeaseContractKey"/>' onclick='return openDialog(this.href, "AgreementInfo", "top=100, left=100, width=700, height=500, scrollbars=yes, resizable=yes");' class='button'>
				...
				<tr class="title">
					<td class="buttons">
						<july:canedit rolesProperty="editRoles">
							<A
								href='SaldoDetailDialog.do?action=Add&idContract=<bean:write name="SaldoDetailListForm" property="mainLeaseContractKey" filter="true"/>'
								onclick='return openDialog(this.href, "SaldoDetailDialog", "<%= propertyDialog%>");'>
	...
	<tr class="title">
		...
		<td class="title" align="center"><A title="Сортировать по полю 'Cальдо внесено вручную'"
	...
	<logic:iterate id="it" name="browseList">
		<logic:present name="it">
			<tr id='<bean:write name="it" property="vo.idSldDetail" filter="true"/>' class="normal" onMouseOver="className='select'" onMouseOut="onMouseOutRow(this.id);" onclick='onClickRow(this.id);'>
				<td class="link">
					<july:canedit rolesProperty="editRoles">
						<A href='SaldoDetailDialog.do?action=Edit&id=<bean:write name="it" property="vo.idSldDetail" filter="true"/>'
							onclick='return openDialog(this.href, "SaldoDetailDialog", "<%= propertyDialog%>");'>
				...
				<td class="text" align="center">
					<logic:present name="it" property="vo.saldoByHand">
						<logic:equal name="it" property="vo.saldoByHand" value="Y">
							<IMG SRC="/images/setrow.gif" BORDER='0' ALT='Cальдо внесено вручную'>
						</logic:equal>
					</logic:present>
				</td>
				<td class="number" align="right">
				...
				<td class="link">
					<july:canedit rolesProperty="editRoles">
						<A href='SaldoDetailDialog.do?action=Delete&id=<bean:write name="it" property="vo.idSldDetail" filter="true"/>'
							onclick='return openDialog(this.href, "SaldoDetailDialog", "<%= propertyDialog%>");'>
