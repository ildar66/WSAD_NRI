...
<% 
	String gColSpan = "15";
	ChargeListLinkToPaymentForm listForm = (ChargeListLinkToPaymentForm) request.getAttribute("ChargeListLinkToPaymentForm");
	String sortBy = listForm.getSortBy();

	String action = "/ChargeListLinkToPayment.do";
	String form = "ChargeListLinkToPaymentForm";
    //start counter:
    //TimeCounter counter = new TimeCounter("PERF.ARENDA.ChargeListLinkToPaymentForm");
    //counter.start();
%>
...
<table class="main" id="tableID">
				...
				<tr class="title">
					...
					<td>
						<A href="javascript:SetPosition()" class='button'>
							Добавить начисление к платежу</A>
					</td>
	...
	<tr class="title">
		...
		<td class="title"><A title="Сортировать по полю 'Курс к валюте начисления'"
			href="javascript:SortBy('remainderRate2Rur')">Курс к валюте начисления</A><%=isUp(sortBy, "remainderRate2Rur")%>
		</td>
		<td class="title"><A title="Сортировать по полю 'Остаток начисления'"
			href="javascript:SortBy('remainderSum')">Остаток начисления</A><%=isUp(sortBy, "remainderSum")%>
		</td>						
	</tr>
	...
	<logic:iterate id="it" name="<%=form%>"	property="chargesList" type="com.hps.july.valueobject.LeaseCharge_TO ">	
		<logic:present name="it">
			<tr
				...
				<td class="number" align="right" nowrap>
					<july:sumwrite name="it" property="vo.remainderRate2Rur"/>
				</td>
				<td class="number" align="right" nowrap>
					<july:sumwrite name="it" property="vo.remainderSum"/>
				</td>																
            </tr>
			<july:separator colspan="<%= gColSpan %>"/>
	      </logic:present>
