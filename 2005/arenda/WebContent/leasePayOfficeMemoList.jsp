...
<%	String gColSpan = "13";
	LeasePayOfficeMemoListForm listForm =
		(LeasePayOfficeMemoListForm) session.getAttribute("LeasePayOfficeMemoListForm");
	String sortBy = listForm.getSortBy();
	String propDialog = "top=140, left=350, width=650, height=550, scrollbars=yes, resizable=yes";
%>
<july:browseform action="/LeasePayOfficeMemoList.do" styleId="listForm">
	...
	<TR class="normal">
		<TD colspan="<%= gColSpan %>">
			<table class="find">
				<tr>
					<td class="text">
						<A href='AgreementInfo.do?action=View&leaseDocument=<bean:write name="LeasePayOfficeMemoListForm" property="mainLeaseContractKey"/>'
					...
					<td class="text">
						<bean:write name="LeasePayOfficeMemoListForm" property="docNumber" filter="true" />
						&nbsp;от&nbsp;<july:datewrite name="LeasePayOfficeMemoListForm" property="docDate"/>
					</td>
		...
		<tr class="title">
			...
			<table class="buttons">
				<tr class="title">
					...
					<A
						href='LeasePayOfficeMemoDialog.do?action=Add&idContract=<bean:write name="LeasePayOfficeMemoListForm" property="mainLeaseContractKey" filter="true"/>'
						onclick='return openDialog(this.href, "LeasePayOfficeMemoDialog", "<%=propDialog%>");'>
		...
		<tr class="title">
			...
			<td class="title"><A
				...
				href="javascript:SortBy('beginDate')">с___</A><%=isUp(sortBy, "beginDate")%>
			</td>
			<td class="title"><A
				title="Сортировать по полю 'Дата окончания периода'"
				href="javascript:SortBy('endDate')">по__</A><%=isUp(sortBy, "endDate")%>
			</td>
			<td class="title"><A title="Сортировать по полю 'Сумма служ.'"
				href="javascript:SortBy('sum')">Сумма</A><%=isUp(sortBy, "sum")%>
			</td>
			<td class="title"><A title="Сортировать по полю 'НДС служ.'"
				href="javascript:SortBy('nds')">НДС</A><%=isUp(sortBy, "nds")%>
			</td>
		...
		<!-- Table body -->
		<logic:iterate id="it" name="browseList" type="LeasePayOfficeMemo_TO">
			<logic:present name="it">
				<tr
					...
					<%
						...
						LeasePayOfficeMemo_TO to = (LeasePayOfficeMemo_TO)it;
					...
					<%--logic:notPresent name="it" property="vo.idAkt"--%>
						<A href='LeasePayOfficeMemoDialog.do?action=<%= actionMemo%>&id=<bean:write name="it" property="vo.idOfficeMemo" filter="true"/>'
							onclick='return openDialog(this.href, "LeasePayOfficeMemoDialog", "<%=propDialog%>");'>
					...
					<td class="text"><july:datewrite name="it" property="vo.beginDate" /></td>
					<td class="text"><july:datewrite name="it" property="vo.endDate" /></td>
					<td class="number" align="right" nowrap><july:sumwrite name="it" property="vo.sum"/></td>
					<td class="number" align="right" nowrap><july:sumwrite name="it" property="vo.nds"/></td>
					<td class="text">			
					...
					<%
						String idStr = "" + ((LeasePayOfficeMemo_TO) it).getVo().getIdOfficeMemo();
						Integer typeDoc = com.hps.july.terrabyte.core.DocumentTypes.LEASE_PAY_OFFICE_MEMO; //тип документа 
						String objectType = com.hps.july.terrabyte.core.ObjectTypes.LEASE_PAY_OFFICE_MEMO; //objectType 
					%>
						<%--logic:notEqual name="it" property="vo.state" value="1"--%>
						<july:wordedit styleId='<%= "wrded1_" + idStr %>' url='<%= "LeasePayOfficeMemoWordAction.do?id=" + idStr%>' 
					...
					<td class="link">
						<A href='LeasePayOfficeMemoDialog.do?action=Add&operation=copy&id=<bean:write name="it" property="vo.idOfficeMemo" filter="true"/>'
							onclick='return openDialog(this.href, "LeasePayOfficeMemoDialog", "<%=propDialog%>");'>COPY</A>
					</td>					
					<td class="text"><july:canedit rolesProperty="editRoles"></july:canedit>
						<logic:equal name="it" property="vo.state" value="1">
						<%--logic:notPresent name="it" property="vo.idAkt"--%>
							 <A href='LeasePayOfficeMemoDialog.do?action=Delete&id=<bean:write name="it" property="vo.idOfficeMemo" filter="true"/>'
								onclick='return openDialog(this.href, "LeasePayOfficeMemoDialog", "<%=propDialog%>");'>
