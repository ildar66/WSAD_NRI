...
<%
	String gColSpan = "12";
	RegistryBuchDocsListForReturnDocumentGroupForm listForm = (RegistryBuchDocsListForReturnDocumentGroupForm)session.getAttribute("RegistryBuchDocsListForReturnDocumentGroupForm");
	...
	String group = null;
	if(isReturnDocumentGroup || request.isUserInRole("administrator")){
		group = AbstractRegistryBuchDocsStateFactory.RG;
	}else if(isArendaPurchaser){
		group = AbstractRegistryBuchDocsStateFactory.PG;
	}else if(isArendaAccountant){
		group = AbstractRegistryBuchDocsStateFactory.AG;
	}	
%>

<july:browseform action="/RegistryBuchDocsListForReturnDocumentGroup.do" styleId="listForm">
...
<table class="main" id="tableID">
			...
			<table class="find" style="font-size:9pt;">
				<tr>
					<td class="text" nowrap colspan="2">
						Типы описи:
						<july:radio property="searchTypeRegistry" insertable="true" updatable="true" value="ALL"/><A href="javascript:toggleSearchTypeRegistry(0)" title="Запросить все описи">
						все</A>&nbsp;
						<july:radio property="searchTypeRegistry" insertable="true" updatable="true" value="1"/><A style='color: green' href="javascript:toggleSearchTypeRegistry(1)" title="Запросить описи счетов-фактуры">
						с.ф.</A>&nbsp;
						<july:radio property="searchTypeRegistry" insertable="true" updatable="true" value="2"/><A style='color: green' href="javascript:toggleSearchTypeRegistry(2)" title="Запросить описи актов выполненных работ">
						акты ВР</A>&nbsp;
						<july:radio property="searchTypeRegistry" insertable="true" updatable="true" value="3"/><A style='color: green' href="javascript:toggleSearchTypeRegistry(3)" title="Запросить описи договоров аренды">
						договора</A>
						<july:radio property="searchTypeRegistry" insertable="true" updatable="true" value="4"/><A style='color: green' href="javascript:toggleSearchTypeRegistry(4)" title="Запросить смешанные описи">
						с.ф. и акты ВР</A>						
					</td>
				</tr>
				<tr>
	...
	<logic:iterate id="it" name="browseList" type="LeaseRegistryBuchDocs_TO">
		...
		<%
			...
			java.util.List statesList = listForm.getRegBuchDocsState().getAvailableStateList(item.getVo().getState(), group);
			...
			<tr id='<bean:write name="it" property="vo.idRegistryDoc" filter="true"/>' class="normal" onMouseOver="className='select'" onMouseOut="onMouseOutRow(this.id);" onclick='onClickRow(this.id);' style="font-size:8pt;">
				...
				<td class="link">
					<logic:equal name="it" property="vo.typeTransferRegistry" value="<%= LeaseRegistryBuchDocs_VO.TYPE_TRANSFER_REGISTRY_REAL%>">									
						<A href='/arenda/RegistryBuchDocsCompositionList.do?isInitialGroup=false&idRegistryDoc=<bean:write name="it" property="vo.idRegistryDoc" filter="true"/>&economist=<bean:write name="it" property="vo.owner" filter="true"/>'><IMG SRC="/images/doc.gif" BORDER='0' ALT='Состав описи'></A>
					</logic:equal>
				</td>
				...
				<td class="text" title="Состояние" align="center">
					<logic:equal name="it" property="vo.typeTransferRegistry" value="<%= LeaseRegistryBuchDocs_VO.TYPE_TRANSFER_REGISTRY_REAL%>">
						<html:select name ="it" property="vo.state" styleId="state" onchange='changeState(this.value);' style="width:125; font-size:9pt">
