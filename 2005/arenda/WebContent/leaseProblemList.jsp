...
{ 
	...
	<TR class="normal">
				...
				<tr>
					<td class="text" colspan="2">
		    			...
		    			<july:select styleClass="seach" onchange="switchFilter()" property="filter" insertable="true" updatable="true">
							...
							<html:option value="1">наименование Исполнителя</html:option>
							<html:option value="2">наименование Инициатора</html:option>
					   	</july:select>
					</td>				
					<td class="text" colspan="2">
		...
		<tr class="title">
			...
			<td class="title"><A title="Сортировать по полю 'Лист позиций'"
				href="javascript:SortBy('listPosContract')">Позиция</A><%=isUp(sortBy, "listPosContract")%>
			</td>
			<td class="title">
				<A title="Сортировать по полю 'Номер договора'"	href="javascript:SortBy('docNumber')">№ дог.</A><%=isUp(sortBy, "docNumber")%><BR>
				<A title="Сортировать по полю 'Дата договора'"	href="javascript:SortBy('docDate')">Дата дог.</A><%=isUp(sortBy, "docDate")%><BR>
			</td>						
		...
		<logic:iterate id="it" name="browseList" type="LeaseContractProblem_TO">
			<logic:present name="it">
				<tr
					...
					<td class="text">
						<bean:write name="it" property="docNumber" filter="true" /> от<BR>
