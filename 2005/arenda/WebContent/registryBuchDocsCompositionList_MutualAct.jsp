...
<tr class="title">
	   <td  class="title">&nbsp;</td>
       <td  class="title">Номер договора</td>
       <td  class="title">Дата договора</td>
       <td  class="title">
       ...
       <td  class="title">
			<A title="Сортировать по полю 'Номер документа'" href="javascript:SortBy('docNumber')">Номер</A><%=isUp(sortBy, "docNumber")%></td>
       <td  class="title">
			<A title="Сортировать по полю 'Дата'" href="javascript:SortBy('docDate')">Дата</A><%=isUp(sortBy, "docDate")%></td>
...
<!-- Table header  end :-->        
        ...
        <logic:iterate id="it" name="browseList" indexId="index" type="RegistryBuchCompositionMutAct_TOC">
            ...
            <tr class="include" onMouseOver="className='select'" onMouseOut="className='include'">
            	<td class="text">
           		<%if(isEdit){%>
                	<input type="checkbox" name="selIds" value='<%= idStr %>' id="selIds" checked="checked" title="Исключить из описи">
             	<%}else{%>
       				<% if(isReturnDocumentGroup){%>
 	            		<A title="Изменить статус документа" href="javascript:revertStateDoc('revertStateDocMutualAct', '<%= idStr %>', '<bean:write name="it" property="actState" filter="true"/>')">
                ...
                <td class="number">
                    <%--bean:write name="it" property="typeCompositionStr" filter="true"/--%><B>Акт ВЗ</B></td>
                <td class="number">
                	<bean:write name="it" property="docNumber" filter="true"/></td>
                <td class="text">
                    <july:datewrite name="it" property="docDate"/></td>
   ...
   <td colspan="<%= gColSpan %>">
      ...
      <tr class="title">
         <td class="buttons">
			...
			<july:backbutton/>
			<B>Номер акта:</B>
			<A href="javascript:toggleStateALL()">ВСЕ</A>&nbsp;&nbsp;&nbsp;
			...
			<july:searchbutton />
			Всего выбрано: <B><bean:write name="listAccessDocsSize" /></B>
         </td>
		 <TH style="text-align: left;">
		 	Пользователь:
		 	<july:checkbox property="isCreatedBy" insertable="true" updatable="true"/>или создал акт&nbsp;&nbsp;
		 	<july:checkbox property="isModifiedBy" insertable="true" updatable="true"/>или изменил акт&nbsp;&nbsp;
		 </TH>
      </tr>
      </TABLE>
   </td>
