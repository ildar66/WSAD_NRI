...
<%
	String gColSpan = "14";
	CompletWorkActsForChargeListForm listForm = (CompletWorkActsForChargeListForm)session.getAttribute("CompletWorkActsForChargeListForm");
	String sortBy = listForm.getSortBy();
%>

<july:browseform action="/CompletWorkActsForChargeList.do" styleId="listForm">
...
function deleteItem(number, idItem) {
	if(confirm('Удалить запись c номером: ' + number+ ' ?')) {
		document.body.style.cursor="wait";
		document.all.operation.value="<%=CompletWorkActsForChargeListForm.operation_DELETE_CWA%>";
...
function changeActState(toActState){
	document.body.style.cursor="wait";
	document.all.operation.value="<%=CompletWorkActsForChargeListForm.operation_CHANGE_ACT_STATE%>";
...
function createCWA(){
	document.body.style.cursor="wait";
	document.all.operation.value="<%=CompletWorkActsForChargeListForm.operation_CREATE_CWA%>";
	listForm.submit();
	return false;
}
</script>
...
<table class="main" id="tableID">
			...
			<table class="buttons">
				<tr class="title">
					<td class="buttons">
						...
						<july:backbutton/>&nbsp;&nbsp;&nbsp;
						<A href='#' onclick='createCWA();' class='button'>
							Сформировать акт выполненных работ</A>
					</td>
					<td align="right">
	...
	<logic:iterate id="it" name="browseList"> 
		...
		<tr id='<bean:write name="it" property="idcwacode" filter="true"/>' class="normal" style="font-size:8pt;" onMouseOver="className='select'" onMouseOut="onMouseOutRow(this.id);" onclick='onClickRow(this.id);'>
			...
			<td class="text">
				<html:select style="width: 120px; font-size: 9pt" name ="it" property="actstate" styleId="actstate" onchange='changeActState(this.value);'>
...
<script language="javascript">
	<%	
		...
		Integer idPosCWA = listForm.getIdPosCWA(); 
		if(operation.equals(CompletWorkActsForChargeListForm.operation_CREATE_CWA) && idPosCWA != null){
	%>
		storeSelectedRow("<%=idPosCWA%>");
		openDialog('CompletWorkAktDialog.do?action=Edit&id=<%=idPosCWA%>', "CompletWorkAktDialog", "top=150, left=470, width=550, height=350, scrollbars=yes, resizable=yes");
	<%}%>
	loadSelectedRow();
	window.location.hash=document.all.selectedID.value;
</script>
