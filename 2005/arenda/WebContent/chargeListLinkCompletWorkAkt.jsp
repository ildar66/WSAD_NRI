...
<% 
	String gColSpan = "15";
	ChargeListLinkToCompletWorkAktForm listForm = (ChargeListLinkToCompletWorkAktForm) request.getAttribute("ChargeListLinkToCompletWorkAktForm");
	String sortBy = listForm.getSortBy();

	String action = "/ChargeListLinkToCompletWorkAkt.do";
	String form = "ChargeListLinkToCompletWorkAktForm";
    //start counter:
    //TimeCounter counter = new TimeCounter("PERF.ARENDA.ChargeListLinkToCompletWorkAktForm");
    //counter.start();
%>
...
function CloseOld()
{
	if(window.opener) {
		...
		window.opener.window.document.forms(0).operation.value="addCWA2charges";
...
function Close()
{
  window.location.href="/arenda/CompletWorkAktProcess.do?operation=addCWA2charges";
}
...
<table class="main" id="tableID">
	...
	<tr class="title">
		...
		<td class="title">&nbsp;</td>
		<td class="title"><A title="Сортировать по полю 'Остаток для акта'"
			href="javascript:SortBy('ostSumForCwAct')">Остаток</A><%=isUp(sortBy, "ostSumForCwAct")%>
		</td>		
	</tr>
	...
	<logic:iterate id="it" name="<%=form%>"	property="chargesList" type="com.hps.july.valueobject.LeaseCharge_TO ">	
		<logic:present name="it">
			<tr
				...
				<td class="number" align="right" nowrap>
					<july:sumwrite name="it" property="vo.ostSumForCwAct"/>
				</td>								
            </tr>
			<july:separator colspan="<%= gColSpan %>"/>
	      </logic:present>
