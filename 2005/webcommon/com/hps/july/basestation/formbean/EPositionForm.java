 ...
 */
public class EPositionForm extends com.hps.july.web.util.EditForm implements Checks {
...
public java.util.Vector getRegions() throws Exception {
    try {
        Vector vec = regions;
        if(regions == null) {
			regions = new RegionObject().findAll(getUsername(), new Boolean(isAdmin()));
        }
        return vec;
    } catch (Exception e) {
 ...
 */
public void initRecord(EquipmentListForm listForm)
	throws Exception
{
 ...
 * Creation date: (13.02.2002 10:25:36)
 */
public void initRecord1(javax.servlet.http.HttpServletRequest request)
    throws Exception {
    DebugSupport.printlnTest("PositionForm initRecord started");
    try {
		setPosstate("P");
