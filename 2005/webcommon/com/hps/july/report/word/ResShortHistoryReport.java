...
/**
 * Печать отчёта "История короткая история перемещений - без учета актов замены"
 */
public class ResShortHistoryReport extends AbstractReport {
	private static java.text.SimpleDateFormat df = new java.text.SimpleDateFormat("dd.MM.yyyy");
public ResShortHistoryReport() {
	super();
}
...
/*
        ...
        s = "SELECT sc.storageplace, sp.type sptype, "+ReportUtils.getStorageNameSQL()+", \n"+
			...
			"serial_for_sc(sc.storagecard) serial, sc.manufserial manuf, x.expeditor exp, x.organization org \n"+
...
//			currentstorage = currentstoragetype + currentstorage;
		...
		s = "SELECT TRIM(r.name) || ' ' || TRIM(r.model) || NVL(' ' || TRIM(sc.configuration),'') agrname, "+
			"serial_for_sc(sc.storagecard) agrserial, sc.manufserial agrmanuf "+
...
*/
		...
		reportNewDataChangeActs(data, con, serialnumber);
		// Reverse documents to be in chronological order
		...
		while (it1.hasNext()) {
			...
			while (it2.hasNext()) {
				...
				if (it3.hasNext()) {
					...
					resname = ro3.getColumn("resname").asString() + ro3.getColumn("serialnumber").asString();
					storagename = "Местоположение: " + ReportUtils.getFullStorageName(stmt, ((Integer)ro3.getColumn("storageplace").asObject()).intValue());
					resmanuf = ro3.getColumn("serialnumber").asString();
					...
					if (it5.hasNext()) {
						...
						doctype = ro5.getColumn("type").asString();
					}
...
// Учет новых данных (после 29.10.2004)
//   по новому алгоритму учета движения оборудования - поиск данных в актах замены
public void reportNewDataChangeActs(java.util.ArrayList data, java.sql.Connection con, String serialnumber)
	throws java.lang.Exception
{
        Statement stmt = con.createStatement();

		String qry0= "SELECT p.storagecard, s.trackid FROM changeactpos p, documents d, storagetracks s " +
			"WHERE p.oldserial = ? AND p.document = d.document AND d.state='2' AND s.document = d.document AND s.operation_type = 2 ";
		CDBCResultSet rs0 = new CDBCResultSet();
		rs0.executeQuery(con, qry0, new Object[] {serialnumber}, 0);
		java.util.ListIterator it0 = rs0.listIterator();
		while (it0.hasNext()) {
			// Определим карточку
			Integer cardid = new Integer(0);
			String resname = "";
			...
			String doctype = "";
			Integer docid = new Integer(0);
			Integer trackid = new Integer(0);
			Integer resource = new Integer(0);
			Integer storageplace = new Integer(0);
			java.sql.Date blankdate = null;
			boolean isFirst = true;
			
			CDBCRowObject ro0 = (CDBCRowObject)it0.next();
			cardid = (Integer)ro0.getColumn("storagecard").asObject();
			trackid = (Integer)ro0.getColumn("trackid").asObject();
				
			String qry2 = "SELECT st.trackid, st.serial, st.inventserial manufserial, st.storageplace, st.document, st.resource, st.operation_type, st.configuration " +
				" FROM storagetracks st " +
				" WHERE st.cardid = ? AND st.parent_trackid IS NULL AND (st.operation_type = 1 OR st.operation_type = 2) AND st.trackid <= ?" +
				" ORDER BY st.trackid DESC ";
			CDBCResultSet rs2 = new CDBCResultSet();
			rs2.executeQuery(con, qry2, new Object[] {cardid, trackid}, 0);

			java.util.ListIterator it2 = rs2.listIterator();
			while (it2.hasNext()) {
				CDBCRowObject ro2 = (CDBCRowObject)it2.next();
				docid = (Integer)ro2.getColumn("document").asObject();
				resource = (Integer)ro2.getColumn("resource").asObject();
				storageplace = (Integer)ro2.getColumn("storageplace").asObject();
				
				String qry3 = "SELECT  " +
				"TRIM(r.name) || NVL(' ' || TRIM(r.model), '') resname " +
				" FROM resources r " +
				" WHERE r.resource = ? ";
				CDBCResultSet rs3 = new CDBCResultSet();
				rs3.executeQuery(con, qry3, new Object[] {resource}, 1);
				java.util.ListIterator it3 = rs3.listIterator();
				if (it3.hasNext()) {
					CDBCRowObject ro3 = (CDBCRowObject)it3.next();
					resname = ro3.getColumn("resname").asString() + ro2.getColumn("configuration").asString() + " S/N: " + ro2.getColumn("serial").asString();
					storagename = "Местоположение: " + ReportUtils.getFullStorageName(stmt, storageplace.intValue());
					...
					if ("2".equals(ro2.getColumn("operation_type").asString())) {
						// Если был расход - найдем соответствующий приход
						storesrc = ReportUtils.getFullStorageName(stmt, storageplace.intValue());
						storedest = "";
						if (!isFirst) {
							String qry4 = "SELECT st.trackid, st.storageplace, st.serial " +
								" FROM storagetracks st " +
								" WHERE st.parent_trackid = ? AND st.operation_type = 1 " +
								" ORDER BY st.trackid ASC ";
							CDBCResultSet rs4 = new CDBCResultSet();
							...
							if (it4.hasNext()) {
								CDBCRowObject ro4 = (CDBCRowObject)it4.next();
								storedest = ReportUtils.getFullStorageName(stmt, ((Integer)ro4.getColumn("storageplace").asObject()).intValue());
								resname = ro3.getColumn("resname").asString() + ro2.getColumn("configuration").asString() + " S/N: " + ro4.getColumn("serial").asString();
								storagename = "Местоположение: " + ReportUtils.getFullStorageName(stmt, ((Integer)ro4.getColumn("storageplace").asObject()).intValue());
								resmanuf = ro4.getColumn("serial").asString();
							}
						}
					} else {
						// Был только приход, без расхода
						storesrc = "";
						storedest = ReportUtils.getFullStorageName(stmt, storageplace.intValue());
					}
