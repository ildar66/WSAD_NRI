...
/**
 * Печать отчёта "Расход с объекта". Подробный отчёт с указанием документов
 */
public class Report062 extends AbstractReport {
	private int docId;
...
public String getComplectSQL() {
	String result =
	"INSERT INTO t_report62 (type1, type2, type3, name, "+
	"conf, unitname, policy, serno, partno, ddate, dtype, dnumber, document, "+
	"price, qty, contract, postavka) "+
	"SELECT rt.name type1, rst.name type2, r.name type3, r.model mname, "+
	"dp.configuration conf, u.shortname unitname, r.countpolicy, serial_for_docpos(dp.docposition), dp.party, d.blankdate ddate, d.type dtype, NVL(d.blankindex || ' ' || d.blanknumber,d.blankindex) dnumber, "+
	"d.document, dp.price price, dp.qty, '', '' "+
	...
	"AND from = "+storageplace+" AND blankdate >= "+getSQLDate(stDate)+" and blankdate <= "+getSQLDate(endDate)+" "+
	"AND state = '2' ) ";
...
public String getPositionSQL() {
	...
	"AND d.from = e.equipment AND e.position = "+storageplace+" AND blankdate >= "+getSQLDate(stDate)+" AND blankdate <= "+getSQLDate(endDate)+" "+
	"AND d.state = '2' )";
//	"GROUP BY rt.name, rst.name, r.name, r.model, dp.configuration, d.blankdate, d.type, d.blanknumber, d.document, dp.price, u.shortname";
    ...
    public void getReportData( Engine engine, String configs ) {
	    System.out.println("In Report062.getReportData");
...
//	    	System.out.println("Cannot get report title");
	    ...
	    try {
			...
			storageplacename = ReportUtils.getStorageName(stmt,storageplace);

	        s = "DROP TABLE t_report62";
			try {
				System.out.println("SQL=" + s);
				stmt.execute(s);
			} catch (Exception e) {
			...
			*/
			...
			s = "UPDATE t_report62 SET dtype = (SELECT CASE d.type "+
				...
				"AND d.document = t_report62.document)";
			...
			if(Boolean.TRUE.equals(showContract)) {
				...
				s = "UPDATE t_report62 "+
					...
					"FROM inwaybills iw "+
					"WHERE iw.document = t_report62.document)";
				System.out.println("R62 SQL="+s);
				try {
					stmt.execute(s);
				} catch(Exception e) {
					e.printStackTrace(System.out);
				...
				*/
				ReportUtils.updateContractsAndSupplies(con,"t_report62");
				s = "SELECT type1, type2, type3, name, "+
					"conf, unitname, ddate, dtype, dnumber, document, "+
					"price, sum(qty) qty, contract, postavka "+
					"FROM t_report62 "+
					"GROUP BY type1, type2, type3, name, conf, unitname, price, contract, postavka, ddate, dtype, dnumber, document "+
					"ORDER BY type1, type2, type3, name, conf, ddate, dtype, dnumber, qty, price";
			} else {
				s = "SELECT type1, type2, type3, name, "+
					"conf, unitname, ddate, dtype, dnumber, document, "+
					"price, sum(qty) qty "+
					"FROM t_report62 "+
					"GROUP BY type1, type2, type3, name, conf, unitname, price, ddate, dtype, dnumber, document "+
					"ORDER BY type1, type2, type3, name, conf, ddate, dtype, dnumber, qty, price";
			}
