 ...
 */
public class AssactReport extends AbstractReport {
	/**
...
public void getReportData( Engine engine, String configs ) {
    ...
    try {
		...
		s = "INSERT INTO t_assacth (document, blanknumber, blankdate, storagecode,   " +
			" storagename, itemtype, iserno, inuser1, pruser) " +
			// "SELECT d.document, NVL(d.blankindex || ' ' || d.blanknumber,d.blankindex), d.blankdate, NVL('D' || p.gsmid || ' ','') || NVL('A' || p.dampsid,''), " +
			"SELECT d.document, TRIM(d.blanknumber), d.blankdate, NVL('D' || p.gsmid || ' ','') || NVL('A' || p.dampsid,''), " +
			" NVL(s1.name, s.name),  " +
...
//			" TRIM(p2.lastname) || ' ' || TRIM(p2.firstname) || ' ' || TRIM(p2.middlename) " +
		...
		s = "INSERT INTO t_assactd (docposition, document, iorder, blocktype, countpolicy, serno, " +
			" price, describe) " +
