...
*/
public class VendorSiteDialogForm extends com.hps.july.web.util.EditForm {
	private int id;
	 ...
	 */
	public int getState() {
		return com.hps.july.dictionary.APPStates.VENDOR_SITE_DIALOG;
	}
	/**
	...
	public void initCollections(javax.servlet.http.HttpServletRequest request) throws Exception {
		...
		setOwners(CDBC_OwnerSystemNFS_Object.getList(getRegionID(), null));
 ...
 * @param request
	...
	private void initOwners(javax.servlet.http.HttpServletRequest request) {
		...
		if (profile.getProfileAccess().getCurrRegion() != null) {
			Integer regID = profile.getProfileAccess().getCurrRegion().getVo().getRegionid();
			setOwners(CDBC_OwnerSystemNFS_Object.getList(regID, null));
		} else {
			//throw new RuntimeException("нет текущего региона для аренды!!!");
			System.out.println("нет текущего региона для аренды!!!");
			owners = Collections.EMPTY_LIST;//new ArrayList();
		}
	}
 */
	/* (non-Javadoc)
