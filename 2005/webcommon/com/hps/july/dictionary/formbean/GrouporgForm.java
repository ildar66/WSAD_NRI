 ...
 */
public class GrouporgForm extends com.hps.july.web.util.EditForm {
...
public com.ibm.ivj.ejb.runtime.AbstractEntityAccessBean constructBean(javax.servlet.http.HttpServletRequest request) throws java.lang.Exception {
	GrouporgListForm aform = (GrouporgListForm)request.getSession().getAttribute( "GrouporgListForm" );
	setSearchgroup(new Integer(aform.getSearchgroup()));
	setSearchgroupname(aform.getSearchgroupname());
	setSearchgroupping(new Integer(aform.getSearchgroupping()));
	setSearchgrouppingname(aform.getSearchgrouppingname());
...
public void fillRecord(com.ibm.ivj.ejb.runtime.AbstractEntityAccessBean bean) {
	try {
		...
		setOrg(abean.getOrganization().getOrganization());
		setOrgname(abean.getOrganization().getName());
	} catch(Exception e) {
	}
}
/**
 ...
 */
public int getState() {
	return com.hps.july.dictionary.APPStates.EDITGRPO;
}
/**
 * Insert the method's description here.
...
{
	if(org <= 0) {
		errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.empty.grouporg.org"));
	} else {
		try {
			...
			errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.invalid.grouporg.org"));
		}
	}
	
	if (searchgroup == null) {
		errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.empty.grouporg.group"));
	}
	else {
		try {
			...
			errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.invalid.grouporg.group"));
		}
		if("Add".equals(getAction())) {
			try {
				...
				errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.dup.grouporg.group"));
			} catch(Exception e) {
			}
		}
	}
	...
	if (searchgroupping == null) {
		errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.empty.grouporg.groupping"));
	}
	else {
		try {
			SearchGrouppingAccessBean bean = new SearchGrouppingAccessBean();
	...
	if (order == null) {
		errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.empty.grouporg.order"));
	}
	else {
		if (order.intValue() <= 0)
			errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.invalid.grouporg.order"));
	}
