 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 * 
 * Обработчик формы списка ресурсов
 */
public class ShowResourcesCommonListAction extends BrowseAction{
	...
	{
		ParamsParser.setState(request, Applications.DICTIONARY, APPStates.COMMONRES_LIST);
		super.perform(mapping, form, request, response);
		ResourcesCommonListForm listform = (ResourcesCommonListForm) form;
		if (listform.getDonotrefresh().booleanValue())
			listform.setDonotrefresh(Boolean.FALSE);
		request.setAttribute("resclass",listform.getResclass());
		request.setAttribute("ResourcesCommonListForm",listform);
