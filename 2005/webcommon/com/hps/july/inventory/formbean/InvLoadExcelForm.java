...
/**
 * Форма загрузки строк документа "Акт инвентаризации на позиции" из файла Excel
 */
public class InvLoadExcelForm extends com.hps.july.web.util.EditForm {
	private int doc;
	private String documentState;
	private String options;
	private Vector data;
	private FormFile psiondataFrm;
public InvLoadExcelForm() {
	options = "1";
}
	...
	throws Exception
{
	...
	for(int i = 0; i < data.size(); i++, order++) {
		...
		if(resource > 0) {
			...
			dp.setIsfictserial(fser);
			if(fser) {
				dp.setSerialnumber(null);
				dp.setRealserial(item[1]);
			} else {
				dp.setSerialnumber(item[1]);
				dp.setRealserial(null);
			}
			dp.setManufserial(item[1]);
			dp.commitCopyHelper();
		}
	}
}
...
public int getState() {
	return com.hps.july.inventory.APPStates.INVLOADEXCEL;
}
/**
