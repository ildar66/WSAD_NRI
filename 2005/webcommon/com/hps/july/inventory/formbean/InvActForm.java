 ...
 */
public class InvActForm extends DocumentForm {
...
public com.ibm.ivj.ejb.runtime.AbstractEntityAccessBean constructBean(javax.servlet.http.HttpServletRequest request) throws java.lang.Exception {
	...
	*/
	...
	I13nActAccessBean bean = new I13nActAccessBean(
		getDocument(), getOwnercode(), getStoragecode(), getStoragecode(),
...
public void fillRecord(AbstractEntityAccessBean bean)
{
	...
	try {
		...
		if(nbean.getFrom() != null) {
			setStoragecode(new Integer(nbean.getFrom().getStorageplace()));
			setStoragename(nbean.getFrom().getName());
		}
...
/**
 * Insert the method's description here.
 * Creation date: (14.07.2005 17:49:47)
 * @return java.lang.String
 */
...
public StoragePlace getFrom() {
	if(storagecode != null) {
		try {
			...
			bean.setInitKey_storageplace(storagecode.intValue());
			bean.refreshCopyHelper();
 ...
 */
public int getState() {
	return com.hps.july.inventory.APPStates.EDITIVA;
}
/**
 * Insert the method's description here.
 * Creation date: (20.05.2002 18:26:46)
 * @return java.lang.Integer
 */
...
/**
 * Insert the method's description here.
 * Creation date: (20.05.2002 18:27:02)
 * @return java.lang.String
 */
	...
	throws Exception
{
	...
	setDocument(0);
	InvActListForm f = (InvActListForm)request.getSession().getAttribute( "InvActListForm" );
	setBlankdate(f.getDateto());
	if(f.getIsowner() == null || !f.getIsowner().booleanValue()) {
		setOwnercode(f.getOwner().getInteger());
		setOwnername(f.getOwnername());
	}
	if(f.getIsstorage() == null || !f.getIsstorage().booleanValue()) {
		setStoragecode(f.getStorage().getInteger());
		try {
			StorageAccessBean b = new StorageAccessBean();
			b.setInitKey_storageplace(getStoragecode().intValue());
			b.refreshCopyHelper();
			setStoragename(b.getName());
		} catch(Exception e) {
			setStoragecode(new Integer(0));
			setStoragename("");
		}
	}
	try {
		...
		setBlanknumber(String.valueOf(getBlankindex()));
	} catch(Exception e) {
		setBlankindex(0);
	}
	dState = "1";
...
/**
 * Insert the method's description here.
 * Creation date: (14.07.2005 17:35:15)
 * @return boolean
 */
...
public void validateValues(ActionErrors errors) throws Exception 
{
	...
	if (storagecode == null) {
		errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.empty.inventory.storage"));
	} else if(storagecode.intValue() <= 0) {
		errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.invalid.inventory.storage"));
	} else {
		try {
			StorageAccessBean s = new StorageAccessBean();
			s.setInitKey_storageplace(storagecode.intValue());
			s.refreshCopyHelper();
