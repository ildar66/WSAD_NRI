...
/**
 * Форма ввода и редактирования шапки документа "Акт монтажа блоков на складе"
 */
public class AssactForm extends DocumentForm {
...
//	private com.hps.july.persistence.WorkerAccessBean monters;
	private java.lang.Integer storagecode;
...
public StorageCardAccessBean constructAgregates()
	throws Exception
{
...
public OrganizationAccessBean constructOrganizations()
	throws Exception
{
...
public StorageAccessBean constructStorages()
	throws Exception
{
...
/**
 * Insert the method's description here.
 * Creation date: (15.07.2005 15:41:33)
 * @return java.lang.String
 */
...
public java.util.ListIterator getListItems() {
	...
	com.hps.july.cdbc.lib.CDBCResultSet rs = c.listStorageMountItems(new Integer(getDocument()),new Integer(1));
 ...
 */
public int getState() {
	return com.hps.july.inventory.APPStates.EDITASS;
}
/**
 * Insert the method's description here.
	...
	throws Exception
{
	...
	ownercode = null;
	storagecode = null;
	montercode.setInteger(null);
	agregatcode = null;
	workercode.setInteger(null);
	montername = "";
	workername = "";
	agregatname = "";
	montertitle = "";
	...
	try {
		AssactListForm aform = (AssactListForm)request.getSession().getAttribute( "AssactListForm" );
...
/**
 * Insert the method's description here.
 * Creation date: (15.07.2005 15:44:50)
 * @return boolean
 */
...
public void validateValues(ActionErrors errors) throws Exception 
{
	...
	if (getOwnercode() == null)
	  errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.empty.assact.owner"));
	else {
		try {
			constructOrganizations();
		} catch (Exception ex) {
			if (ErrorProcessor.isRecordNotExistException(ex))
			  errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.invalid.assact.owner"));
		}
	}
	
	if (getStoragecode() == null)
	  errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.empty.assact.storage"));
	else {
		try {
			constructStorages();
		} catch (Exception ex) {
			if (ErrorProcessor.isRecordNotExistException(ex))
			  errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.invalid.assact.storage"));
		}
	}
	...
	if (blankdate.isEmpty())
	  errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.empty.assact.date"));
	else if (!blankdate.isOk())
	  errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.invalid.assact.date"));
	
	/*if ((agregateserial == null) || (agregateserial.length() == 0))
	  errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.empty.assact.agregat"));
	else {
		try {
			constructAgregates();
		} catch (Exception ex) {
			if (ErrorProcessor.isRecordNotExistException(ex))
			  errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.invalid.assact.agregat"));
		}
	}*/
	
	if (!workercode.isEmpty())
	    if (!workercode.isOk())
	       errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.invalid.assact.worker"));
		else {
			try {
				constructWorkers();
			} catch (Exception ex) {
	...
	if (montercode.isEmpty())
	  errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.empty.assact.monter"));
	else if (!montercode.isOk())
	  errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.invalid.assact.monter"));
	else {
		try {
			constructMonters();
		} catch (Exception ex) {
