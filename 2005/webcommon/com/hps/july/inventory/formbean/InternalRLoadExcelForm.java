...
/**
 * ‘орма загрузки строк документа "¬нутренн€€ расходна€ накладна€" из файла Excel
 */
public class InternalRLoadExcelForm extends com.hps.july.web.util.EditForm {
	private int doc;
	private String documentState;
	private String options;
	private Vector data;
	private FormFile psiondataFrm;
public InternalRLoadExcelForm() {
	options = "1";
}
	...
	throws Exception
{
	...
	for(int i = 0; i < data.size(); i++, order++) {
		...
		try {
			...
			if(resource > 0) {		
				dp = new DocumentPositionAccessBean(
					docposition,
...
public int getState() {
	return com.hps.july.inventory.APPStates.IORLOADEXCEL;
}
/**
