...
/**
 * Обработчик формы списка строк документа "Акты демонтажа блоков на позиции"
 */
public class ShowDAssactposBListAction
...
public ActionForward perform(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    ParamsParser.setState(request, Applications.INVENTORY, APPStates.DASSBPLIST);
	DAssactposBListForm aform = (DAssactposBListForm)form;
	...
	if(aform.getDocument() != null) {
		try {
			...
			DAssactBValue v = new DAssactBValue(bean);
			...
			if(com.hps.july.util.AppUtils.getInventoryVariant() == 2) {
				aform.setAgregateserial(bean.getAgregateserial());
				...
				s = s + ", № " + bean.getAgregat().getSerialnumber();
				aform.setAgregatename(s);
				aform.setAgregateserial(bean.getAgregat().getSerialnumber());
			}
