...
/**
 * Для загрузки файлов Word из внешних источников.
 * Creation date: (17.11.2005 10:08:47)
 * @author: Shafigullin Ildar
 */
public class DocumentWordDownloadServlet extends javax.servlet.http.HttpServlet {
	/**
	...
	public void performTask(
		...
		Integer id = null; //код документа
		String objectTypes = null; //тип документа

		try {
			id = new Integer((String) request.getParameter("id"));
			objectTypes = (String) request.getParameter("objectTypes");
			System.out.println("DocumentWordDownloadServlet: id=" + id + "; objectTypes=" + objectTypes);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		if (id != null) {
			try {
				...
				try {
					WordDocument doc = DocumentWordDownload_Object.loadWordExternal(id, objectTypes, request.getRemoteUser());
					...
					if (text != null && text.length > 0 && name != null && !name.equals("")) {
						response.setContentType(
							"application/msword; charset=windows-1251; name=" + java.net.URLEncoder.encode(name));
						...
						String s =
							"<html>"
								+ "<head><title>Докумен Word</title><link rel=\"stylesheet\" type=\"text/css\" href=\"/main.css\"></head>"
