 ...
 */
public class SchetFactWordUploadAction extends com.hps.july.web.util.AbstractDocumentUploadAction {
/**
...
public String storeInDatabase(ActionMapping mapping, DocumentUploadForm form, HttpServletRequest request, HttpServletResponse response) {
    ...
    if (!form.isEmpty()) {
        try {
            ...
            if (isCDBCstory) {
                if (!CDBC_SchetFact_Object.storeWord(form.getDocumentid(), form.getFilebody().getFileName(), form.getFilebody().getFileData()))
                    result = "Ошибка сохранения в SchetFactWordUploadAction";
            } else {
                //Сохрание с внешними библиотеками:
                CDBC_SchetFact_Object.storeWordExternal(
            ...
            result = "Ошибка сохранения в SchetFactWordUploadAction:" + e.getMessage();
        }
    }
    return result;
}
