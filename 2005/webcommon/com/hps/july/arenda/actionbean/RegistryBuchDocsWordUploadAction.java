 ...
 */
public class RegistryBuchDocsWordUploadAction extends com.hps.july.web.util.AbstractDocumentUploadAction {
/**
...
public String storeInDatabase(ActionMapping mapping, DocumentUploadForm form, HttpServletRequest request, HttpServletResponse response) {
    ...
    if (!form.isEmpty()) {
        try {
            ...
            if (isCDBCstory) {
                if (!CDBC_LeaseRegistryBuchDocs_Object.storeWord(form.getDocumentid(), form.getFilebody().getFileName(), form.getFilebody().getFileData()))
                    result = "Ошибка сохранения в RegistryBuchDocsWordUploadAction";
            } else {
                //Сохрание с внешними библиотеками:
				CDBC_LeaseRegistryBuchDocs_Object.storeWordExternal(
            ...
            result = "Ошибка сохранения в RegistryBuchDocsWordUploadAction:" + e.getMessage();
        }
    }
    return result;
}
