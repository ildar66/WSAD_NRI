...
/**
 * «Проблемы ведения договоров» BrowseForm Object.
 * Creation date: (12.10.2005 15:40:48)
 * @author: Shafigullin Ildar
 */
public class LeaseContractProblemListForm extends com.hps.july.web.util.BrowseForm {
	 ...
	 */
	public LeaseContractProblemListForm() {
		super();
	...
	public java.lang.Object[] getFilterParams() {
		return new Object[] { getMainLeaseContractKey(), getState().equals("ALL") ? Boolean.FALSE : Boolean.TRUE, getState(), new Integer(getFilter()), getSearchString(), getSortBy()};
	}
	/**
	...
	public ActionForward initBrowse(ActionMapping mapping, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		ParamsParser.setState(request, Applications.ARENDA, APPStates.LEASE_CONTRACT_PROBLEM_LIST);
		initAgreementInfo();
	...
	private void changeState(ActionErrors errors) {
		...
		if (!errorTxt.equals("")) {
			...
			//"Передана на устранение"
			if(getToState().equals("2")){
				try {
					CDBC_LeaseContractProblem_Object.sendMail(getOperatorCode(), getIdCode(), this.principalName);
				} catch (Exception e) {
					errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.txt", "Проблемы с отправкой сообщения исполнителю!!!"));
					e.printStackTrace();
				}
			}
		}
