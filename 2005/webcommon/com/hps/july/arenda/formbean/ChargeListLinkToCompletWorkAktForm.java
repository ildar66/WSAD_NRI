 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class ChargeListLinkToCompletWorkAktForm extends ChargeListForm {
	 ...
	 * 
	 */
	public ChargeListLinkToCompletWorkAktForm() {
		super();
	}
	 ...
	 */
	public int getState() {
		return com.hps.july.arenda.APPStates.CHARGE_LIST_LINK_TO_COMPLET_WORK_AKT;
	}
	...
	public void initCollections(HttpServletRequest request) throws Exception {
		...
		if (getSelectIds().length > 0) {
			...
			//добавляем позиции в форму "CompletWorkAktDialogForm":
			CompletWorkAktDialogForm cwaForm = (CompletWorkAktDialogForm) request.getSession().getAttribute("CompletWorkAktDialogForm");
			for (Iterator iterator = getChargesList().iterator(); iterator.hasNext();) {
				...
				if (selIds.contains(lch_vo.getLeaseDocPosition())) {
					CompletWorkAkt2Charge_TO to = new CompletWorkAkt2Charge_TO();
					to.setChargeVO(lch_vo);
					to.setLinkSumCharge(lch_vo.getOstSumForCwAct());
					to.setCurName(lch_to.getCurName());
					to.setResName(lch_to.getResName());

					cwaForm.getCwa2charges().add(to);
				}
			}
	...
	private void initChargesList(HttpServletRequest request) throws Exception {
		...
		if (getLeaseContract() != 0) {
			...
			list = new ArrayList();
			CDBCResultSet rSet =
				CDBCLeaseChargesObject.findListLinkToCompletWorkAkt(
					Boolean.TRUE,
