...
/**
 * Form-Bean "Служебных записок для ссудных договоров"
 * Creation date: (26.10.2005 12:10:30)
 * @author: Shafigullin Ildar
 */
public class LoanContractMemoForm extends EditForm {
...
/**
 * Insert the method's description here.
 * Creation date: (26.10.2005 18:30:50)
 * @return int
 */
 ...
 */
public int getState() {
	return com.hps.july.arenda.APPStates.LOAN_CONTRACT_MEMO;
}
/**
 * Insert the method's description here.
...
/**
 * Insert the method's description here.
 * Creation date: (28.10.2005 16:18:05)
 * @param memo com.hps.july.arenda.valueobject.Memo_VO
 */
private void initAbonentContract(int aID) {
    LoanContractVO voLC = CDBC_LoanContract_Object.find(aID);
    Integer orgKey = voLC.getOrgCustomer().getOrganization();
    int abonentContract;
    if (CDBC_Person_Object.find(orgKey.intValue()) != null) {
    ...
    if (abonentContract != 0)
        memo.setAbonentContract(new Integer(abonentContract));
    else
...
public void initRecord(javax.servlet.http.HttpServletRequest request) throws Exception {
	...
	getMemo().setLoanContract(new Integer(getLoanContract()));
	getMemo().setManVO(getManVO_fromOperatorCode(request));
	getMemo().setListMemoPos(new ArrayList());
	getMemo().setType("E");//
	initAbonentContract(getLoanContract());
	//инициализируем позицию:
	memoPos = new MemoPos_VO();
}
/**
...
public void loadRecord(javax.servlet.http.HttpServletRequest request) throws Exception {
	...
	if (ro != null) {
		memo = new Memo_VO(ro);
		memo.setLoanContract((Integer)ro.getColumn("loanContract").asObject());
		getMemo().setListMemoPos(getListMemoPosForMemo(memo.getIdHeader()));
		setLoanContract(memo.getLoanContract().intValue());
		//инициализируем позицию:
		memoPos = new MemoPos_VO();
...
public String processDelete(javax.servlet.http.HttpServletRequest request, ActionErrors errors) throws Exception {
	try {
		...
		System.out.println("LoanContractMemoForm.processDelete():errorTxt: " + e);
...
public String processInsert(javax.servlet.http.HttpServletRequest request, ActionErrors errors) throws Exception {
	...
	try {
		...
		System.out.println("LoanContractMemoForm.processInsert():errorTxt: " + e);
		errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.txt", e));
		e.printStackTrace();
	}
	return null;
...
public String processUpdate(javax.servlet.http.HttpServletRequest request, ActionErrors errors) throws Exception {
    ...
    try {
        ...
        System.out.println("LoanContractMemoForm.processUpdate():errorTxt: " + e);
        errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.txt", e));
        e.printStackTrace();
    }
    return null;
