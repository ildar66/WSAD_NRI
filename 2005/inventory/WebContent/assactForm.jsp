...
<%
	String masterTagId = "assactFormWordEditMasterTagId";
%>
<july:wordEditMaster styleId='<%= masterTagId %>' >
</july:wordEditMaster>

<july:editform action="/ProcessAssact.do">
<bean:define id="docCode" name="AssactForm" property="document" scope="session" type="java.lang.Integer"/>
...
<%
	AssactForm frm = (AssactForm)request.getSession().getAttribute("AssactForm");
...
--%>	
<july:wordEditSlave masterStyleId='<%= masterTagId %>' styleId='<%= "assactForm_" + theDocCode %>' 
		url='<%= "reports/assact.jsp?prompt1=" + theDocCode %>' 
			uploadurl=''
			updatable='false'
...
&nbsp;&nbsp;
	...
	<%} else {%>
		<html:link page="/ProcessAssact.do?action=Edit&processState=C" paramId="document" paramName="AssactForm" paramProperty="document" paramScope="session" onclick="return checkPosted();"><img src="/images/calculates.gif" border=0 alt="Снять обсчёт"></html:link>
	<%}%>
<%}%>
</july:formbuttons>
...
<july:separator/>
    ...
    <tr>
        <td class="text"><july:reqmark><bean:message key="label.assact.owner"/>:</july:reqmark></td>
        <td>
    ...
    <tr>
        <td class="text"><july:reqmark><bean:message key="label.assact.storage"/>:</july:reqmark></td>
        <td>
...
<%}%>
    ...
    <tr>
        <td class="text"><july:reqmark><bean:message key="label.assact.num"/>:</july:reqmark></td>
        <td>
    ...
    <tr>
        <td class="text"><july:reqmark><bean:message key="label.assact.date"/>:</july:reqmark></td>
        <td>
...
<july:separator/>	
    <tr>
		...
		<july:lookupbutton action="/ShowCardLookupList.do" styleId="srl" onclick="set_opt_params_srl('?pagetype=2&owner=&storage='+document.all.storagecode.value+'&resource=&agregate=&brokentype=1&documentkey=')" fieldNames="temp;agregatname;temp;temp;agregateserial;temp;temp;temp;temp;temp;temp;temp" newWindowName="equiplookup" insertable="true" updatable="true"/>
		<%--<july:string size="27" property="agregatname" styleId="agregatname" insertable="false" updatable="false" />--%>
        </td>
    </tr>

    <july:separator/>
    
    <logic:equal name="AssactForm" property="action" value="View">
    ...
    <logic:notEqual name="AssactForm" property="action" value="View">
    
        <tr>
            <td class="text"><bean:message key="label.assact.config"/>:</td>
            <td>
               <july:string property="oldconfig" size="25" insertable="false" updatable="false" />
            </td>
        </tr>
    ...
    <tr>
        <td class="text"><bean:message key="label.assact.stworker"/>:</td>
        <td>
    ...
    <tr>
        <td class="text"><july:reqmark/><bean:message key="label.assact.monter"/>:</july:reqmark></td>
        <td>
...
<tr class="title">
	...
	<tr>
		<td><nobr><%if(changable) {%><july:addbutton page="/EditAssactpos.do?action=Add" alttext="label.assactpos.add" onclick="return checkPosted();"/><%}%></nobr></td>
		<td class="title" align="right">
			<bean:message key="label.assact.positions"/>
		</td>
	</tr>
	</table>
	</td>
...
<tr><td colspan="2" style="padding:0px;">
	...
	<logic:iterate id="it" name="AssactForm" property="listItems" type="com.hps.july.cdbc.lib.CDBCRowObject">
...
*/
	...
	<tr class="normal" onMouseOver="className='select';" onMouseOut="className='normal';">
		<td class="string">
			<%if(changable) {%>
				<july:editbutton page="/EditAssactpos.do" action="Edit" paramId="docposition" paramName="docposition" paramScope="page" alttext="label.assactpos.edit"/>
			<%} else {%>
				<july:editbutton page="/EditAssactpos.do" action="View" paramId="docposition" paramName="docposition" paramScope="page" alttext="label.assactpos.view"/>
			<%}%>
		</td>
		...
		<%if(changable) {%><td class="link"><july:delbutton page="/EditAssactpos.do?action=Delete" paramId="docposition" paramName="it" paramProperty="docposition" paramScope="page" /></td><%}%>
	</tr>
	</logic:present>
	</logic:iterate>
	</table>
