...
<bean:write name="DivisionLookupListForm" property="companyName"/>:&nbsp;

<%
DivisionLookupListForm form = (DivisionLookupListForm)session.getAttribute("DivisionLookupListForm");
int countLevel = form.getCountLevel();
int[] history = form.getHistory();
for(int i=0; i<=countLevel; i++){
	if(i != countLevel){%>
	<a href="ShowDivisionLookupList.do?upparent=<%=history[i]%>&countLevel=<%=i%>">
	<% if(i == 0){%>
