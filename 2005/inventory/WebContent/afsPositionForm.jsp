...
<july:editform action="/ProcessAfsPosition.do">

<%	
	AfsPositionForm dform = (AfsPositionForm)request.getAttribute("AfsPositionForm");
	...
	<td>
		<logic:equal name="AfsForm" property="readonly" value="false">
			...
			<%} else {
				...
				%>
				<july:lookupbutton action="<%= s + dform.getSnumber() %>" styleId="resl" fieldNames="resourcecode;resourcemodel;resourcename" newWindowName="resourcelookup" insertable="true" updatable="false" image="lookup_red"/>
			<%}%>
		...
		<logic:equal name="AfsForm" property="readonly" value="true">
			...
			<%} else {
				String s = "/ShowResourceLookupBySerialList.do?snumber=";
				%>
				<july:lookupbutton action="<%= s + dform.getSnumber() %>" styleId="resl" fieldNames="resourcecode;resourcemodel;resourcename" newWindowName="resourcelookup" insertable="false" updatable="false" image="lookup_red"/>
			<%}%>
...
function checkFictionSerial() {
	if(document.all.action.value == 'Add' || document.all.action.value == 'Edit') {
		switchLookup('snumber','ser',document.all.fictionSerial[0].checked);
			...
			<july:string property="snumber" size="15" insertable="true" updatable='true' /> <%-- <%=""+dform.isSerialUpdatable()%> --%>
...
<tr>
	...
	<td>
		<logic:equal name="AfsForm" property="readonly" value="false">
			<july:string property="party" size="15" insertable="true" updatable="true" /> <%--=""+dform.isPartyUpdatable()--%>
