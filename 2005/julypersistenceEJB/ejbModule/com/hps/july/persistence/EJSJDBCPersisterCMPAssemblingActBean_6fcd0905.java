...
/**
 * EJSJDBCPersisterCMPAssemblingActBean_6fcd0905
 */
public class EJSJDBCPersisterCMPAssemblingActBean_6fcd0905 extends EJSJDBCPersister implements com.hps.july.persistence.EJSFinderAssemblingActBean, com.ibm.vap.finders.VapEJSJDBCRawFinderStatementHelper {
	private static final String[] _createString = {"INSERT INTO documents (document, insuranceact, blankdate, blanknumber, parentdocument, operator, isautodoc, blankindex, processsource, showtouser, state, processdestination, insurancedate, insuranceman, to2, needapprove, projectid, prjstatus, visualtype, owner, to, contragent, from, storagemanager, type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", "INSERT INTO asemblingacts (document, agregateserial, newconfiguration, unmountblocks, oldconfiguration, operationtype, agregateresource, inventserial, localserial, monter, agregat) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"};
	private static final String[] _removeString = {"DELETE FROM asemblingacts  WHERE document = ?", "DELETE FROM documents  WHERE document = ?"};
	private static final String[] _storeString = {"UPDATE asemblingacts  SET agregateserial = ?, newconfiguration = ?, unmountblocks = ?, oldconfiguration = ?, operationtype = ?, agregateresource = ?, inventserial = ?, localserial = ?, monter = ?, agregat = ? WHERE document = ?", "UPDATE documents  SET insuranceact = ?, blankdate = ?, blanknumber = ?, parentdocument = ?, operator = ?, isautodoc = ?, blankindex = ?, processsource = ?, showtouser = ?, state = ?, processdestination = ?, insurancedate = ?, insuranceman = ?, to2 = ?, needapprove = ?, projectid = ?, prjstatus = ?, visualtype = ?, owner = ?, to = ?, contragent = ?, from = ?, storagemanager = ?, type = ? WHERE document = ?"};
	private static final String _loadString = "SELECT T2.insuranceact, T2.blankdate, T2.blanknumber, T2.parentdocument, T2.operator, T2.isautodoc, T2.blankindex, T2.processsource, T2.showtouser, T2.state, T2.processdestination, T2.insurancedate, T2.insuranceman, T2.to2, T2.document, T2.needapprove, T2.projectid, T2.prjstatus, T2.visualtype, T2.owner, T2.to, T2.contragent, T2.from, T2.storagemanager, T2.type, T1.agregateserial, T1.newconfiguration, T1.unmountblocks, T1.oldconfiguration, T1.operationtype, T1.agregateresource, T1.inventserial, T1.localserial, T1.monter, T1.agregat FROM documents  T2, asemblingacts  T1 WHERE T2.type = \'A\' AND T2.document = T1.document AND T2.document = ?";
	private static final String _loadForUpdateString = _loadString + " FOR UPDATE";
	private byte[] serObj = null;
	/**
	 * EJSJDBCPersisterCMPAssemblingActBean_6fcd0905
	 */
	public EJSJDBCPersisterCMPAssemblingActBean_6fcd0905() throws java.rmi.RemoteException {
		super();	}
	/**
	 * postInit
	 */
	...
	public void _create(EntityBean eb) throws Exception {
		...
		AssemblingActBean b = (AssemblingActBean) eb;
		...
		try {
			...
			objectTemp = com.ibm.vap.converters.VapTrimStringConverter.singleton().dataFrom(b.newConfiguration);
			if (objectTemp == null) {
			...
			objectTemp = com.ibm.vap.converters.VapTrimStringConverter.singleton().dataFrom(b.oldConfiguration);
			if (objectTemp == null) {
			...
			objectTemp = com.ibm.vap.converters.VapTrimStringConverter.singleton().dataFrom(b.operationType);
			if (objectTemp == null) {
			...
			if (b.monter_worker == null) {
				...
				pstmt.setInt(10, b.monter_worker.intValue());
			}
			if (b.agregat_storagecard == null) {
				pstmt.setNull(11, java.sql.Types.INTEGER);
			}
			else {
				pstmt.setInt(11, b.agregat_storagecard.intValue());
			}
			pstmt.setInt(1, b.document);
			pstmt.executeUpdate();
		}
	...
	public void hydrate(EntityBean eb, Object data, Object pKey) throws Exception {
		...
		b.newConfiguration = (java.lang.String)com.ibm.vap.converters.VapTrimStringConverter.singleton().objectFrom(resultSet.getString(27));
		b.unmountblocks = (java.lang.Boolean)com.ibm.vap.converters.VapStringToBooleanConverter.singleton().objectFrom(resultSet.getString(28));
		b.oldConfiguration = (java.lang.String)com.ibm.vap.converters.VapTrimStringConverter.singleton().objectFrom(resultSet.getString(29));
		b.operationType = (java.lang.String)com.ibm.vap.converters.VapTrimStringConverter.singleton().objectFrom(resultSet.getString(30));
		tempint = resultSet.getInt(31);
		b.agregateresource = resultSet.wasNull() ? null : new Integer(tempint);
		b.inventserial = (java.lang.String)com.ibm.vap.converters.VapTrimStringConverter.singleton().objectFrom(resultSet.getString(32));
		b.localserial = (java.lang.String)com.ibm.vap.converters.VapTrimStringConverter.singleton().objectFrom(resultSet.getString(33));
		tempint = resultSet.getInt(34);
		b.monter_worker = resultSet.wasNull() ? null : new Integer(tempint);
		tempint = resultSet.getInt(35);
		b.agregat_storagecard = resultSet.wasNull() ? null : new Integer(tempint);
	...
	public void store(EntityBean eb) throws Exception {
		...
		try {
			pstmt.setInt(11, _primaryKey.document);
			objectTemp = com.ibm.vap.converters.VapTrimStringConverter.singleton().dataFrom(b.agregateserial);
			if (objectTemp == null) {
			...
			if (b.monter_worker == null) {
				...
				pstmt.setInt(9, b.monter_worker.intValue());
			}
			if (b.agregat_storagecard == null) {
				pstmt.setNull(10, java.sql.Types.INTEGER);
			}
			else {
				pstmt.setInt(10, b.agregat_storagecard.intValue());
			}
			pstmt.executeUpdate();
		}
		finally {
...
return null;
	 ...
	 */
	public EJSFinder findAssemblingActByMonter(com.hps.july.persistence.WorkerKey inKey) throws javax.ejb.FinderException, java.rmi.RemoteException {
		ResultSet resultSet = null;
		PreparedStatement pstmt = null;
		try {
			preFind();
			pstmt = getPreparedStatement("SELECT T2.insuranceact, T2.blankdate, T2.blanknumber, T2.parentdocument, T2.operator, T2.isautodoc, T2.blankindex, T2.processsource, T2.showtouser, T2.state, T2.processdestination, T2.insurancedate, T2.insuranceman, T2.to2, T2.document, T2.needapprove, T2.projectid, T2.prjstatus, T2.visualtype, T2.owner, T2.to, T2.contragent, T2.from, T2.storagemanager, T2.type, T1.agregateserial, T1.newconfiguration, T1.unmountblocks, T1.oldconfiguration, T1.operationtype, T1.agregateresource, T1.inventserial, T1.localserial, T1.monter, T1.agregat FROM documents  T2, asemblingacts  T1 WHERE T2.type = \'A\' AND T2.document = T1.document AND T1.monter = ?");
			Object objectTemp = null;
boolean nullData;
	 ...
	 */
	public EJSFinder findAssemblingActByAgregat(com.hps.july.persistence.StorageCardKey inKey) throws javax.ejb.FinderException, java.rmi.RemoteException {
		ResultSet resultSet = null;
		PreparedStatement pstmt = null;
		try {
			preFind();
			pstmt = getPreparedStatement("SELECT T2.insuranceact, T2.blankdate, T2.blanknumber, T2.parentdocument, T2.operator, T2.isautodoc, T2.blankindex, T2.processsource, T2.showtouser, T2.state, T2.processdestination, T2.insurancedate, T2.insuranceman, T2.to2, T2.document, T2.needapprove, T2.projectid, T2.prjstatus, T2.visualtype, T2.owner, T2.to, T2.contragent, T2.from, T2.storagemanager, T2.type, T1.agregateserial, T1.newconfiguration, T1.unmountblocks, T1.oldconfiguration, T1.operationtype, T1.agregateresource, T1.inventserial, T1.localserial, T1.monter, T1.agregat FROM documents  T2, asemblingacts  T1 WHERE T2.type = \'A\' AND T2.document = T1.document AND T1.agregat = ?");
			Object objectTemp = null;
boolean nullData;
	...
	public com.hps.july.persistence.AssemblingAct findByPrimaryKey(com.hps.july.persistence.DocumentKey key) throws java.rmi.RemoteException, javax.ejb.FinderException {
		ResultSet resultSet = null;
		PreparedStatement pstmt = null;
		com.hps.july.persistence.AssemblingAct result = null;

		EJSJDBCFinder tmpFinder = null;
		com.hps.july.persistence.DocumentKey _primaryKey = (com.hps.july.persistence.DocumentKey)key;
		try {
			try {
				result = (com.hps.july.persistence.AssemblingAct)home.getBean(key);
			}
			catch (Throwable ex) {
				result=null;
			}
			if (result == null) {
				...
				if (tmpFinder.hasMoreElements()) {
					result = (com.hps.july.persistence.AssemblingAct)tmpFinder.nextElement();
				}
			}
		}
		catch (Exception ex) {
	 ...
	 * findByQBE2
	 */
	public EJSFinder findByQBE2(java.sql.Date startDate, java.sql.Date endDate, java.lang.Boolean isOrganization, java.lang.Integer organization, java.lang.Boolean isPosition, java.lang.Integer position, java.lang.Boolean isOperationType, java.lang.String operationType, java.lang.Integer order) throws javax.ejb.FinderException, java.rmi.RemoteException {
		ResultSet resultSet = null;
		PreparedStatement pstmt = null;
		try {
			preFind();
			pstmt = getFinderObject().findByQBE2(startDate, endDate, isOrganization, organization, isPosition, position, isOperationType, operationType, order);
			resultSet = pstmt.executeQuery();
			return new EJSJDBCFinder(resultSet, this, pstmt);
		}
		catch (Exception ex) {
	 ...
	 * findByQBE
	 */
	public EJSFinder findByQBE(java.sql.Date startDate, java.sql.Date endDate, java.lang.Boolean isOrganization, java.lang.Integer organization, java.lang.Boolean isStorage, java.lang.Integer storage, java.lang.Boolean isOperationType, java.lang.String operationType, java.lang.Integer order) throws javax.ejb.FinderException, java.rmi.RemoteException {
		ResultSet resultSet = null;
		PreparedStatement pstmt = null;
		try {
			preFind();
			pstmt = getFinderObject().findByQBE(startDate, endDate, isOrganization, organization, isStorage, storage, isOperationType, operationType, order);
			resultSet = pstmt.executeQuery();
			return new EJSJDBCFinder(resultSet, this, pstmt);
		}
		catch (Exception ex) {
	...
	private static final String genericFindSqlString = "SELECT T2.insuranceact, T2.blankdate, T2.blanknumber, T2.parentdocument, T2.operator, T2.isautodoc, T2.blankindex, T2.processsource, T2.showtouser, T2.state, T2.processdestination, T2.insurancedate, T2.insuranceman, T2.to2, T2.document, T2.needapprove, T2.projectid, T2.prjstatus, T2.visualtype, T2.owner, T2.to, T2.contragent, T2.from, T2.storagemanager, T2.type, T1.agregateserial, T1.newconfiguration, T1.unmountblocks, T1.oldconfiguration, T1.operationtype, T1.agregateresource, T1.inventserial, T1.localserial, T1.monter, T1.agregat FROM documents  T2, asemblingacts  T1 WHERE T2.type = \'A\' AND T2.document = T1.document AND ";
	private static final int[] genericFindInsertPoints = {630};
	/**
	...
	private AssemblingActBeanFinderObject finderObject = null;
	/**
	 * getFinderObject
	 */
	public com.hps.july.persistence.AssemblingActBeanFinderObject getFinderObject() {
		if (finderObject == null) {
			com.hps.july.persistence.AssemblingActBeanFinderObject temp_finderObject = new com.hps.july.persistence.AssemblingActBeanFinderObject();
