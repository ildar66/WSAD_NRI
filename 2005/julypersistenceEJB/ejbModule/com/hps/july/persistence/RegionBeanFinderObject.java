 ...
 */
public class RegionBeanFinderObject extends com.ibm.vap.finders.VapEJSJDBCFinderObject  
		implements RegionBeanFinderHelper{
 ...
 * @return java.sql.PreparedStatement
 */
public PreparedStatement findByQBE(Boolean isSuperRegion, Integer superRegion, 
    ...
    int[] genericFindInsertPoints = getGenericFindInsertPoints();
    StringBuffer sb = new StringBuffer(getGenericFindSqlString());
    String alias = getAlias(sb.toString(), "REGIONS");
    
    //calculate where clause
	StringBuffer whereClause = new StringBuffer( " 1=1 " );
	if( isSuperRegion.booleanValue() )
		whereClause.append( " AND " + alias + ".supregid = ? ");
	if( isRegionName.booleanValue() )
		whereClause.append( " AND UPPER(" + alias + ".regname) MATCHES UPPER(?) ");
	...
	for( i = 0; i < getMergedWhereCount(); i++) {
		if( isSuperRegion.booleanValue() )
			ps.setInt( j++, superRegion.intValue() );			
		if( isRegionName.booleanValue() )
			ps.setString( j++, regionName );			
	}
...
public static java.lang.String getNumberOrder(String statement, String alias ) {
	if( numberOrder == null ) {
		numberOrder = String.valueOf( PersistenceUtil.getFieldIndex( statement, alias+".regionid" ) );
	}
	return numberOrder;
}
/**
