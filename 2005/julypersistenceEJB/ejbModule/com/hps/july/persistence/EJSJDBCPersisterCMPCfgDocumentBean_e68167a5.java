...
/**
 * EJSJDBCPersisterCMPCfgDocumentBean_e68167a5
 */
public class EJSJDBCPersisterCMPCfgDocumentBean_e68167a5 extends EJSJDBCPersister implements com.hps.july.persistence.EJSFinderCfgDocumentBean {
	private static final String[] _createString = {"INSERT INTO documents (document, insuranceact, blankdate, blanknumber, parentdocument, operator, isautodoc, blankindex, processsource, showtouser, state, processdestination, insurancedate, insuranceman, to2, needapprove, projectid, prjstatus, visualtype, owner, to, contragent, from, storagemanager, type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", "INSERT INTO cfgdocument (document, savconfigid_old, savconfigid_new, storedem, storebad) VALUES (?, ?, ?, ?, ?)"};
	private static final String[] _removeString = {"DELETE FROM cfgdocument  WHERE document = ?", "DELETE FROM documents  WHERE document = ?"};
	private static final String[] _storeString = {"UPDATE cfgdocument  SET savconfigid_old = ?, savconfigid_new = ?, storedem = ?, storebad = ? WHERE document = ?", "UPDATE documents  SET insuranceact = ?, blankdate = ?, blanknumber = ?, parentdocument = ?, operator = ?, isautodoc = ?, blankindex = ?, processsource = ?, showtouser = ?, state = ?, processdestination = ?, insurancedate = ?, insuranceman = ?, to2 = ?, needapprove = ?, projectid = ?, prjstatus = ?, visualtype = ?, owner = ?, to = ?, contragent = ?, from = ?, storagemanager = ?, type = ? WHERE document = ?"};
	private static final String _loadString = "SELECT T2.insuranceact, T2.blankdate, T2.blanknumber, T2.parentdocument, T2.operator, T2.isautodoc, T2.blankindex, T2.processsource, T2.showtouser, T2.state, T2.processdestination, T2.insurancedate, T2.insuranceman, T2.to2, T2.document, T2.needapprove, T2.projectid, T2.prjstatus, T2.visualtype, T2.owner, T2.to, T2.contragent, T2.from, T2.storagemanager, T2.type, T1.savconfigid_old, T1.savconfigid_new, T1.storedem, T1.storebad FROM documents  T2, cfgdocument  T1 WHERE T2.type = \'G\' AND T2.document = T1.document AND T2.document = ?";
	private static final String _loadForUpdateString = _loadString + " FOR UPDATE";
	private byte[] serObj = null;
	/**
	 * EJSJDBCPersisterCMPCfgDocumentBean_e68167a5
	 */
	public EJSJDBCPersisterCMPCfgDocumentBean_e68167a5() throws java.rmi.RemoteException {
		super();	}
	/**
	 * postInit
	 */
	...
	public void _create(EntityBean eb) throws Exception {
		...
		CfgDocumentBean b = (CfgDocumentBean) eb;
		...
		try {
			pstmt.setInt(2, b.savconfigidold);
			pstmt.setInt(3, b.savconfigidnew);
			if (b.storedem == null) {
				pstmt.setNull(4, java.sql.Types.INTEGER);
			}
			else {
				pstmt.setInt(4, b.storedem.intValue());
			}
			if (b.storebad == null) {
				pstmt.setNull(5, java.sql.Types.INTEGER);
			}
			else {
				pstmt.setInt(5, b.storebad.intValue());
			}
			pstmt.setInt(1, b.document);
			pstmt.executeUpdate();
		}
	...
	public void hydrate(EntityBean eb, Object data, Object pKey) throws Exception {
		...
		b.savconfigidold = resultSet.getInt(26);
		b.savconfigidnew = resultSet.getInt(27);
		tempint = resultSet.getInt(28);
		b.storedem = resultSet.wasNull() ? null : new Integer(tempint);
		tempint = resultSet.getInt(29);
		b.storebad = resultSet.wasNull() ? null : new Integer(tempint);
	...
	public void store(EntityBean eb) throws Exception {
		...
		try {
			...
			pstmt.setInt(1, b.savconfigidold);
			pstmt.setInt(2, b.savconfigidnew);
			if (b.storedem == null) {
				pstmt.setNull(3, java.sql.Types.INTEGER);
			}
			else {
				pstmt.setInt(3, b.storedem.intValue());
			}
			if (b.storebad == null) {
				pstmt.setNull(4, java.sql.Types.INTEGER);
			}
			else {
				pstmt.setInt(4, b.storebad.intValue());
			}
			pstmt.executeUpdate();
		}
		finally {
...
return null;
	 ...
	 * findByPrimaryKey
	 */
	public com.hps.july.persistence.CfgDocument findByPrimaryKey(com.hps.july.persistence.DocumentKey primaryKey) throws java.rmi.RemoteException, javax.ejb.FinderException {
		ResultSet resultSet = null;
		PreparedStatement pstmt = null;
		com.hps.july.persistence.CfgDocument result = null;

		EJSJDBCFinder tmpFinder = null;
		com.hps.july.persistence.DocumentKey _primaryKey = (com.hps.july.persistence.DocumentKey)primaryKey;
		try {
			try {
				result = (com.hps.july.persistence.CfgDocument)home.getBean(primaryKey);
			}
			catch (Throwable ex) {
				result=null;
			}
			if (result == null) {
				...
				if (tmpFinder.hasMoreElements()) {
					result = (com.hps.july.persistence.CfgDocument)tmpFinder.nextElement();
				}
			}
		}
		catch (Exception ex) {
