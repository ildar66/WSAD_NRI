...
/**
 * EJSJDBCPersisterCMPAfsMountActBean_cf191c99
 */
public class EJSJDBCPersisterCMPAfsMountActBean_cf191c99 extends EJSJDBCPersister implements com.hps.july.persistence.EJSFinderAfsMountActBean, com.ibm.vap.finders.VapEJSJDBCRawFinderStatementHelper {
	private static final String[] _createString = {"INSERT INTO documents (document, insuranceact, blankdate, blanknumber, parentdocument, operator, isautodoc, blankindex, processsource, showtouser, state, processdestination, insurancedate, insuranceman, to2, needapprove, projectid, prjstatus, visualtype, owner, to, contragent, from, storagemanager, type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", "INSERT INTO internalwaybills (document, fixing, equiptype, position, monter) VALUES (?, ?, ?, ?, ?)", "INSERT INTO afsmountact (document, afstype) VALUES (?, ?)"};
	private static final String[] _removeString = {"DELETE FROM afsmountact  WHERE document = ?", "DELETE FROM internalwaybills  WHERE document = ?", "DELETE FROM documents  WHERE document = ?"};
	private static final String[] _storeString = {"UPDATE afsmountact  SET afstype = ? WHERE document = ?", "UPDATE internalwaybills  SET fixing = ?, equiptype = ?, position = ?, monter = ? WHERE document = ?", "UPDATE documents  SET insuranceact = ?, blankdate = ?, blanknumber = ?, parentdocument = ?, operator = ?, isautodoc = ?, blankindex = ?, processsource = ?, showtouser = ?, state = ?, processdestination = ?, insurancedate = ?, insuranceman = ?, to2 = ?, needapprove = ?, projectid = ?, prjstatus = ?, visualtype = ?, owner = ?, to = ?, contragent = ?, from = ?, storagemanager = ?, type = ? WHERE document = ?"};
	private static final String _loadString = "SELECT T2.insuranceact, T2.blankdate, T2.blanknumber, T2.parentdocument, T2.operator, T2.isautodoc, T2.blankindex, T2.processsource, T2.showtouser, T2.state, T2.processdestination, T2.insurancedate, T2.insuranceman, T2.to2, T2.document, T2.needapprove, T2.projectid, T2.prjstatus, T2.visualtype, T2.owner, T2.to, T2.contragent, T2.from, T2.storagemanager, T2.type, T3.fixing, T3.equiptype, T3.position, T3.monter, T1.afstype FROM afsmountact  T1, documents  T2, internalwaybills  T3 WHERE T2.type = \'B\' AND T2.document = T3.document AND T3.document = T1.document AND T2.document = ?";
	private static final String _loadForUpdateString = _loadString + " FOR UPDATE";
	private byte[] serObj = null;
	/**
	 * EJSJDBCPersisterCMPAfsMountActBean_cf191c99
	 */
	public EJSJDBCPersisterCMPAfsMountActBean_cf191c99() throws java.rmi.RemoteException {
		super();	}
	/**
	 * postInit
	 */
	...
	public void _create(EntityBean eb) throws Exception {
		...
		AfsMountActBean b = (AfsMountActBean) eb;
		...
		try {
			objectTemp = com.ibm.vap.converters.VapTrimStringConverter.singleton().dataFrom(b.afstype);
			if (objectTemp == null) {
				pstmt.setNull(2, java.sql.Types.CHAR);
			}
			else {
	...
	public void hydrate(EntityBean eb, Object data, Object pKey) throws Exception {
		...
		b.afstype = (java.lang.String)com.ibm.vap.converters.VapTrimStringConverter.singleton().objectFrom(resultSet.getString(30));
...
return null;
	...
	public com.hps.july.persistence.AfsMountAct findByPrimaryKey(com.hps.july.persistence.DocumentKey key) throws java.rmi.RemoteException, javax.ejb.FinderException {
		ResultSet resultSet = null;
		PreparedStatement pstmt = null;
		com.hps.july.persistence.AfsMountAct result = null;

		EJSJDBCFinder tmpFinder = null;
		com.hps.july.persistence.DocumentKey _primaryKey = (com.hps.july.persistence.DocumentKey)key;
		try {
			try {
				result = (com.hps.july.persistence.AfsMountAct)home.getBean(key);
			}
			catch (Throwable ex) {
				result=null;
			}
			if (result == null) {
				...
				if (tmpFinder.hasMoreElements()) {
					result = (com.hps.july.persistence.AfsMountAct)tmpFinder.nextElement();
				}
			}
		}
		catch (Exception ex) {
	...
	private static final int[] genericFindInsertPoints = {567};
	/**
	...
	private AfsMountActBeanFinderObject finderObject = null;
	/**
	 * getFinderObject
	 */
	public com.hps.july.persistence.AfsMountActBeanFinderObject getFinderObject() {
		if (finderObject == null) {
			com.hps.july.persistence.AfsMountActBeanFinderObject temp_finderObject = new com.hps.july.persistence.AfsMountActBeanFinderObject();
