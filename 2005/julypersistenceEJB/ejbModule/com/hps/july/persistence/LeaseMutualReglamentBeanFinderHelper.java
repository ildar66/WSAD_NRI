 ...
 **/
public interface LeaseMutualReglamentBeanFinderHelper extends com.hps.july.persistence.LeaseDocumentBeanFinderHelper {
	public static final String findConcurentReglamentsByReglamentWhereClause = "t1.leasedocument IN (" +
     "SELECT rs.leasedocument "+
     " FROM leasemutualrglmnt rs, leasemutcontracts ms,   "+
     " leasemutualrglmnt ro, leasemutcontracts mo WHERE "+
     " ms.reglament = rs.leasedocument AND mo.reglament = ro.leasedocument "+
     " AND ro.leasedocument = ? AND " + 
     " ms.contract = mo.contract AND ms.resource = mo.resource AND " +
     "("+ 
      "(rs.startdate BETWEEN ro.startdate AND ro.enddate) OR "+
      "(rs.enddate BETWEEN  ro.startdate AND ro.enddate) " +
     ")"+ 
     " AND ( rs.mutstate = 'B' OR rs.mutstate = 'C') " +
		 " AND rs.leasedocument <> ro.leasedocument )";
	public java.sql.PreparedStatement findByLeaseDocumentOrderByDocumentNumberAsc(Integer leaseDocument) throws Exception;
	...
	java.sql.Date startDate, java.sql.Date finishDate) throws Exception;
