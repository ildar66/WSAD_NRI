 ...
 **/
public interface WorkerBeanFinderHelper { 
	public static final String findByLastNameOrderByLastNameAscWhereClause = "man in (select man from people where upper(lastname) matches upper(?) )";
	public static final String findAlienWorkersWhereClause = "isactive='Y' AND isour='N' AND organization = ? AND man in (select man from people where upper(lastname) matches upper(?) ) ORDER BY worker ASC";
	public java.sql.PreparedStatement findByQBE(Boolean isDivision, Integer argDivision,
	  Boolean isLastName, String argLastName,
	  Boolean isActive, Boolean argActive, Integer order) throws Exception;
}
