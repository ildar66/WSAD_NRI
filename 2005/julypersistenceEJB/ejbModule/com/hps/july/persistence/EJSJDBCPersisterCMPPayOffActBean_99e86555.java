...
/**
 * EJSJDBCPersisterCMPPayOffActBean_99e86555
 */
public class EJSJDBCPersisterCMPPayOffActBean_99e86555 extends EJSJDBCPersister implements com.hps.july.persistence.EJSFinderPayOffActBean, com.ibm.vap.finders.VapEJSJDBCRawFinderStatementHelper {
	private static final String[] _createString = {"INSERT INTO documents (document, insuranceact, blankdate, blanknumber, parentdocument, operator, isautodoc, blankindex, processsource, showtouser, state, processdestination, insurancedate, insuranceman, to2, needapprove, projectid, prjstatus, visualtype, owner, to, contragent, from, storagemanager, type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", "INSERT INTO payoffacts (document, chairman, member1, member2) VALUES (?, ?, ?, ?)"};
	private static final String[] _removeString = {"DELETE FROM payoffacts  WHERE document = ?", "DELETE FROM documents  WHERE document = ?"};
	private static final String[] _storeString = {"UPDATE payoffacts  SET chairman = ?, member1 = ?, member2 = ? WHERE document = ?", "UPDATE documents  SET insuranceact = ?, blankdate = ?, blanknumber = ?, parentdocument = ?, operator = ?, isautodoc = ?, blankindex = ?, processsource = ?, showtouser = ?, state = ?, processdestination = ?, insurancedate = ?, insuranceman = ?, to2 = ?, needapprove = ?, projectid = ?, prjstatus = ?, visualtype = ?, owner = ?, to = ?, contragent = ?, from = ?, storagemanager = ?, type = ? WHERE document = ?"};
	private static final String _loadString = "SELECT T1.insuranceact, T1.blankdate, T1.blanknumber, T1.parentdocument, T1.operator, T1.isautodoc, T1.blankindex, T1.processsource, T1.showtouser, T1.state, T1.processdestination, T1.insurancedate, T1.insuranceman, T1.to2, T1.document, T1.needapprove, T1.projectid, T1.prjstatus, T1.visualtype, T1.owner, T1.to, T1.contragent, T1.from, T1.storagemanager, T1.type, T2.chairman, T2.member1, T2.member2 FROM payoffacts  T2, documents  T1 WHERE T1.type = \'P\' AND T1.document = T2.document AND T1.document = ?";
	private static final String _loadForUpdateString = _loadString + " FOR UPDATE";
	private byte[] serObj = null;
	/**
	 * EJSJDBCPersisterCMPPayOffActBean_99e86555
	 */
	public EJSJDBCPersisterCMPPayOffActBean_99e86555() throws java.rmi.RemoteException {
		super();	}
	/**
	 * postInit
	 */
	...
	public void _create(EntityBean eb) throws Exception {
		...
		PayOffActBean b = (PayOffActBean) eb;
		...
		try {
			if (b.chairMan_worker == null) {
				pstmt.setNull(2, java.sql.Types.INTEGER);
			}
			else {
				pstmt.setInt(2, b.chairMan_worker.intValue());
			}
			if (b.member1_worker == null) {
				pstmt.setNull(3, java.sql.Types.INTEGER);
			}
			else {
				pstmt.setInt(3, b.member1_worker.intValue());
			}
			if (b.member2_worker == null) {
				pstmt.setNull(4, java.sql.Types.INTEGER);
			}
			else {
				pstmt.setInt(4, b.member2_worker.intValue());
			}
			pstmt.setInt(1, b.document);
			pstmt.executeUpdate();
		}
	...
	public void hydrate(EntityBean eb, Object data, Object pKey) throws Exception {
		...
		b.chairMan_worker = resultSet.wasNull() ? null : new Integer(tempint);
		tempint = resultSet.getInt(27);
		b.member1_worker = resultSet.wasNull() ? null : new Integer(tempint);
		tempint = resultSet.getInt(28);
		b.member2_worker = resultSet.wasNull() ? null : new Integer(tempint);
	...
	public void store(EntityBean eb) throws Exception {
		...
		try {
			pstmt.setInt(4, _primaryKey.document);
			if (b.chairMan_worker == null) {
				pstmt.setNull(1, java.sql.Types.INTEGER);
			}
			else {
				pstmt.setInt(1, b.chairMan_worker.intValue());
			}
			if (b.member1_worker == null) {
				pstmt.setNull(2, java.sql.Types.INTEGER);
			}
			else {
				pstmt.setInt(2, b.member1_worker.intValue());
			}
			if (b.member2_worker == null) {
				pstmt.setNull(3, java.sql.Types.INTEGER);
			}
			else {
				pstmt.setInt(3, b.member2_worker.intValue());
			}
			pstmt.executeUpdate();
		}
		finally {
...
return null;
	 ...
	 */
	public EJSFinder findPayOffActMemberByMember1(com.hps.july.persistence.WorkerKey inKey) throws javax.ejb.FinderException, java.rmi.RemoteException {
		ResultSet resultSet = null;
		PreparedStatement pstmt = null;
		try {
			preFind();
			pstmt = getPreparedStatement("SELECT T1.insuranceact, T1.blankdate, T1.blanknumber, T1.parentdocument, T1.operator, T1.isautodoc, T1.blankindex, T1.processsource, T1.showtouser, T1.state, T1.processdestination, T1.insurancedate, T1.insuranceman, T1.to2, T1.document, T1.needapprove, T1.projectid, T1.prjstatus, T1.visualtype, T1.owner, T1.to, T1.contragent, T1.from, T1.storagemanager, T1.type, T2.chairman, T2.member1, T2.member2 FROM payoffacts  T2, documents  T1 WHERE T1.type = \'P\' AND T1.document = T2.document AND T2.member1 = ?");
			Object objectTemp = null;
boolean nullData;
	 ...
	 */
	public EJSFinder findPayOffActChairManByChairMan(com.hps.july.persistence.WorkerKey inKey) throws javax.ejb.FinderException, java.rmi.RemoteException {
		ResultSet resultSet = null;
		PreparedStatement pstmt = null;
		try {
			preFind();
			pstmt = getPreparedStatement("SELECT T1.insuranceact, T1.blankdate, T1.blanknumber, T1.parentdocument, T1.operator, T1.isautodoc, T1.blankindex, T1.processsource, T1.showtouser, T1.state, T1.processdestination, T1.insurancedate, T1.insuranceman, T1.to2, T1.document, T1.needapprove, T1.projectid, T1.prjstatus, T1.visualtype, T1.owner, T1.to, T1.contragent, T1.from, T1.storagemanager, T1.type, T2.chairman, T2.member1, T2.member2 FROM payoffacts  T2, documents  T1 WHERE T1.type = \'P\' AND T1.document = T2.document AND T2.chairman = ?");
			Object objectTemp = null;
boolean nullData;
			pstmt.setInt(1, inKey.worker);
			resultSet = pstmt.executeQuery();
	...
	public com.hps.july.persistence.PayOffAct findByPrimaryKey(com.hps.july.persistence.DocumentKey key) throws java.rmi.RemoteException, javax.ejb.FinderException {
		ResultSet resultSet = null;
		PreparedStatement pstmt = null;
		com.hps.july.persistence.PayOffAct result = null;

		EJSJDBCFinder tmpFinder = null;
		com.hps.july.persistence.DocumentKey _primaryKey = (com.hps.july.persistence.DocumentKey)key;
		try {
			try {
				result = (com.hps.july.persistence.PayOffAct)home.getBean(key);
			}
			catch (Throwable ex) {
				result=null;
			}
			if (result == null) {
				...
				if (tmpFinder.hasMoreElements()) {
					result = (com.hps.july.persistence.PayOffAct)tmpFinder.nextElement();
				}
			}
		}
		catch (Exception ex) {
	 ...
	 */
	public EJSFinder findPayOffActMember2ByMember2(com.hps.july.persistence.WorkerKey inKey) throws javax.ejb.FinderException, java.rmi.RemoteException {
		ResultSet resultSet = null;
		PreparedStatement pstmt = null;
		try {
			preFind();
			pstmt = getPreparedStatement("SELECT T1.insuranceact, T1.blankdate, T1.blanknumber, T1.parentdocument, T1.operator, T1.isautodoc, T1.blankindex, T1.processsource, T1.showtouser, T1.state, T1.processdestination, T1.insurancedate, T1.insuranceman, T1.to2, T1.document, T1.needapprove, T1.projectid, T1.prjstatus, T1.visualtype, T1.owner, T1.to, T1.contragent, T1.from, T1.storagemanager, T1.type, T2.chairman, T2.member1, T2.member2 FROM payoffacts  T2, documents  T1 WHERE T1.type = \'P\' AND T1.document = T2.document AND T2.member2 = ?");
			Object objectTemp = null;
boolean nullData;
			pstmt.setInt(1, inKey.worker);
			resultSet = pstmt.executeQuery();
	...
	private PayOffActBeanFinderObject finderObject = null;
	/**
	 * getFinderObject
	 */
	public com.hps.july.persistence.PayOffActBeanFinderObject getFinderObject() {
		if (finderObject == null) {
			com.hps.july.persistence.PayOffActBeanFinderObject temp_finderObject = new com.hps.july.persistence.PayOffActBeanFinderObject();
