...
/**
 * EJSJDBCPersisterCMPLeaseResourceBean_37b5eaeb
 */
public class EJSJDBCPersisterCMPLeaseResourceBean_37b5eaeb extends EJSJDBCPersister implements com.hps.july.persistence.EJSFinderLeaseResourceBean {
	private static final String[] _createString = {"INSERT INTO resources (resource, notes, actualcode, countpolicy, manucode, complectpart, active, complect, boxable, name, model, priceable, resourceclass2, groupid, manufid, disablenfscodes, resourcesubtype, unit, resourceclass) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", "INSERT INTO leaseresources (resource, needcwact, useinarendacontract, useinabonentcontract) VALUES (?, ?, ?, ?)"};
	private static final String[] _removeString = {"DELETE FROM leaseresources  WHERE resource = ?", "DELETE FROM resources  WHERE resource = ?"};
	private static final String[] _storeString = {"UPDATE leaseresources  SET needcwact = ?, useinarendacontract = ?, useinabonentcontract = ? WHERE resource = ?", "UPDATE resources  SET notes = ?, actualcode = ?, countpolicy = ?, manucode = ?, complectpart = ?, active = ?, complect = ?, boxable = ?, name = ?, model = ?, priceable = ?, resourceclass2 = ?, groupid = ?, manufid = ?, disablenfscodes = ?, resourcesubtype = ?, unit = ?, resourceclass = ? WHERE resource = ?"};
	private static final String _loadString = "SELECT T2.notes, T2.actualcode, T2.countpolicy, T2.resource, T2.manucode, T2.complectpart, T2.active, T2.complect, T2.boxable, T2.name, T2.model, T2.priceable, T2.resourceclass2, T2.groupid, T2.manufid, T2.disablenfscodes, T2.resourcesubtype, T2.unit, T2.resourceclass, T1.needcwact, T1.useinarendacontract, T1.useinabonentcontract FROM leaseresources  T1, resources  T2 WHERE T2.resourceclass = \'X\' AND T2.resource = T1.resource AND T2.resource = ?";
	private static final String _loadForUpdateString = _loadString + " FOR UPDATE";
	private byte[] serObj = null;
	/**
	 * EJSJDBCPersisterCMPLeaseResourceBean_37b5eaeb
	 */
	public EJSJDBCPersisterCMPLeaseResourceBean_37b5eaeb() throws java.rmi.RemoteException {
		super();	}
	/**
	 * postInit
	 */
	...
	public void _create(EntityBean eb) throws Exception {
		...
		LeaseResourceBean b = (LeaseResourceBean) eb;
		...
		try {
			objectTemp = com.ibm.vap.converters.VapStringToBooleanConverter.singleton().dataFrom(new Boolean(b.needCwAct));
			if (objectTemp == null) {
			...
			objectTemp = com.ibm.vap.converters.VapStringToBooleanConverter.singleton().dataFrom(new Boolean(b.useInArendaContract));
			if (objectTemp == null) {
			...
			objectTemp = com.ibm.vap.converters.VapStringToBooleanConverter.singleton().dataFrom(new Boolean(b.useInAbonentContract));
			if (objectTemp == null) {
				pstmt.setNull(4, java.sql.Types.CHAR);
			}
			else {
	...
	public void hydrate(EntityBean eb, Object data, Object pKey) throws Exception {
		...
		b.needCwAct = (objectTemp == null) ? false : ((Boolean)objectTemp).booleanValue();
		objectTemp = com.ibm.vap.converters.VapStringToBooleanConverter.singleton().objectFrom(resultSet.getString(21));
		b.useInArendaContract = (objectTemp == null) ? false : ((Boolean)objectTemp).booleanValue();
		objectTemp = com.ibm.vap.converters.VapStringToBooleanConverter.singleton().objectFrom(resultSet.getString(22));
		b.useInAbonentContract = (objectTemp == null) ? false : ((Boolean)objectTemp).booleanValue();
...
return null;
	 ...
	 * findByPrimaryKey
	 */
	public com.hps.july.persistence.LeaseResource findByPrimaryKey(com.hps.july.persistence.ResourceKey primaryKey) throws java.rmi.RemoteException, javax.ejb.FinderException {
		ResultSet resultSet = null;
		PreparedStatement pstmt = null;
		com.hps.july.persistence.LeaseResource result = null;

		EJSJDBCFinder tmpFinder = null;
		com.hps.july.persistence.ResourceKey _primaryKey = (com.hps.july.persistence.ResourceKey)primaryKey;
		try {
			try {
				result = (com.hps.july.persistence.LeaseResource)home.getBean(primaryKey);
			}
			catch (Throwable ex) {
				result=null;
			}
			if (result == null) {
				...
				if (tmpFinder.hasMoreElements()) {
					result = (com.hps.july.persistence.LeaseResource)tmpFinder.nextElement();
				}
			}
		}
		catch (Exception ex) {
