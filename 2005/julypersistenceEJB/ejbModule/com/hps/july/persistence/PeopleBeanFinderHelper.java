 ...
 **/
public interface PeopleBeanFinderHelper { 
	public static final String findByNameOrderByNameAscWhereClause = "upper(lastname) matches upper(?)  and isActive = 'Y' order by lastname asc";			
	public static final String findByNameOrderByNameDescWhereClause = "upper(lastname) matches upper(?)  and isActive = 'Y' order by lastname desc";			
	public static final String findMaxKeyValueWhereClause = "man = (select max(man) from people)";		
	public static final String findWorkerByLastNameOrderByLastNameAscWhereClause =
		"upper(t1.lastname) matches upper(?) and exists (select w.worker from workers w where w.man=t1.man)  and isActive = 'Y' order by t1.lastname asc";

	public java.sql.PreparedStatement findByConnectedOrderByNameAsc( Integer argConnected ) throws Exception;
