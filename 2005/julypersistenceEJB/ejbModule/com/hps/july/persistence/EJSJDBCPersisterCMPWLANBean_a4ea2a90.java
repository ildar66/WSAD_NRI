...
/**
 * EJSJDBCPersisterCMPWLANBean_a4ea2a90
 */
public class EJSJDBCPersisterCMPWLANBean_a4ea2a90 extends EJSJDBCPersister implements com.hps.july.persistence.EJSFinderWLANBean, com.ibm.vap.finders.VapEJSJDBCRawFinderStatementHelper {
	private static final String[] _createString = {"INSERT INTO storageplaces (storageplace, closedate, address, operator, name, modified, created, modifiedby, createdby, division, type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", "INSERT INTO equipment (equipment, state, date_bldbeg, pwr_val, pwr_res_cat, date_assemb, date_test, date_pr, pwr_cat, equipcomment, pwr_reserve, datasource, position) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", "INSERT INTO wlan (equipment, number) VALUES (?, ?)"};
	private static final String[] _removeString = {"DELETE FROM wlan  WHERE equipment = ?", "DELETE FROM equipment  WHERE equipment = ?", "DELETE FROM storageplaces  WHERE storageplace = ?"};
	private static final String[] _storeString = {"UPDATE wlan  SET number = ? WHERE equipment = ?", "UPDATE equipment  SET state = ?, date_bldbeg = ?, pwr_val = ?, pwr_res_cat = ?, date_assemb = ?, date_test = ?, date_pr = ?, pwr_cat = ?, equipcomment = ?, pwr_reserve = ?, datasource = ?, position = ? WHERE equipment = ?", "UPDATE storageplaces  SET closedate = ?, address = ?, operator = ?, name = ?, modified = ?, created = ?, modifiedby = ?, createdby = ?, division = ?, type = ? WHERE storageplace = ?"};
	private static final String _loadString = "SELECT T2.closedate, T2.address, T2.operator, T2.name, T2.storageplace, T2.modified, T2.created, T2.modifiedby, T2.createdby, T2.division, T2.type, T1.state, T1.date_bldbeg, T1.pwr_val, T1.pwr_res_cat, T1.date_assemb, T1.date_test, T1.date_pr, T1.pwr_cat, T1.equipcomment, T1.pwr_reserve, T1.datasource, T1.position, T3.number FROM storageplaces  T2, wlan  T3, equipment  T1 WHERE T2.type = \'L\' AND T1.equipment = T3.equipment AND T2.storageplace = T1.equipment AND T2.storageplace = ?";
	private static final String _loadForUpdateString = _loadString + " FOR UPDATE";
	private byte[] serObj = null;
	/**
	 * EJSJDBCPersisterCMPWLANBean_a4ea2a90
	 */
	public EJSJDBCPersisterCMPWLANBean_a4ea2a90() throws java.rmi.RemoteException {
		super();	}
	/**
	 * postInit
	 */
...
return null;
		...
		try {
			...
			if (result == null) {
				...
				if (tmpFinder.hasMoreElements()) {
					result = (com.hps.july.persistence.WLAN)tmpFinder.nextElement();
				}
			}
		}
		catch (Exception ex) {
	...
	private static final String genericFindSqlString = "SELECT T2.closedate, T2.address, T2.operator, T2.name, T2.storageplace, T2.modified, T2.created, T2.modifiedby, T2.createdby, T2.division, T2.type, T1.state, T1.date_bldbeg, T1.pwr_val, T1.pwr_res_cat, T1.date_assemb, T1.date_test, T1.date_pr, T1.pwr_cat, T1.equipcomment, T1.pwr_reserve, T1.datasource, T1.position, T3.number FROM storageplaces  T2, wlan  T3, equipment  T1 WHERE T2.type = \'L\' AND T1.equipment = T3.equipment AND T2.storageplace = T1.equipment AND ";
	private static final int[] genericFindInsertPoints = {466};
	/**
	...
	private WLANBeanFinderObject finderObject = null;
	/**
	 * getFinderObject
	 */
	public com.hps.july.persistence.WLANBeanFinderObject getFinderObject() {
		if (finderObject == null) {
			com.hps.july.persistence.WLANBeanFinderObject temp_finderObject = new com.hps.july.persistence.WLANBeanFinderObject();
