...
/**
 * EJSJDBCPersisterCMPUZOPowerSupplySetBean_d36d4692
 */
public class EJSJDBCPersisterCMPUZOPowerSupplySetBean_d36d4692 extends EJSJDBCPersister implements com.hps.july.persistence.EJSFinderUZOPowerSupplySetBean {
	private static final String[] _createString = {"INSERT INTO resourcesets (resourceset, mainpartqty, type, name, resourcesetclass, nfsitem_id, mainpart) VALUES (?, ?, ?, ?, ?, ?, ?)", "INSERT INTO uzopowersupplyset (resourceset) VALUES (?)"};
	private static final String[] _removeString = {"DELETE FROM uzopowersupplyset  WHERE resourceset = ?", "DELETE FROM resourcesets  WHERE resourceset = ?"};
	...
	private static final String _loadString = "SELECT T1.mainpartqty, T1.resourceset, T1.type, T1.name, T1.resourcesetclass, T1.nfsitem_id, T1.mainpart FROM resourcesets  T1, uzopowersupplyset  T2 WHERE T1.resourcesetclass = 3 AND T1.resourceset = T2.resourceset AND T1.resourceset = ?";
	private static final String _loadForUpdateString = _loadString + " FOR UPDATE";
	private byte[] serObj = null;
	/**
	 * EJSJDBCPersisterCMPUZOPowerSupplySetBean_d36d4692
	 */
	public EJSJDBCPersisterCMPUZOPowerSupplySetBean_d36d4692() throws java.rmi.RemoteException {
		super();	}
	/**
	 * postInit
	 */
	...
	public void _create(EntityBean eb) throws Exception {
		...
		UZOPowerSupplySetBean b = (UZOPowerSupplySetBean) eb;
...
return null;
	 ...
	 * findByPrimaryKey
	 */
	public com.hps.july.persistence.UZOPowerSupplySet findByPrimaryKey(com.hps.july.persistence.ResourceSetKey primaryKey) throws java.rmi.RemoteException, javax.ejb.FinderException {
		ResultSet resultSet = null;
		PreparedStatement pstmt = null;
		com.hps.july.persistence.UZOPowerSupplySet result = null;

		EJSJDBCFinder tmpFinder = null;
		com.hps.july.persistence.ResourceSetKey _primaryKey = (com.hps.july.persistence.ResourceSetKey)primaryKey;
		try {
			try {
				result = (com.hps.july.persistence.UZOPowerSupplySet)home.getBean(primaryKey);
			}
			catch (Throwable ex) {
				result=null;
			}
			if (result == null) {
				...
				if (tmpFinder.hasMoreElements()) {
					result = (com.hps.july.persistence.UZOPowerSupplySet)tmpFinder.nextElement();
				}
			}
		}
		catch (Exception ex) {
