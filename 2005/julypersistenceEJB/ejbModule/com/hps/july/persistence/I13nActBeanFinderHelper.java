 ...
 **/
public interface I13nActBeanFinderHelper extends com.hps.july.persistence.DocumentBeanFinderHelper {
	public static final String findFullByStorageAfterDateWhereClause = 
		"t1.from = ? and t1.blankdate >= ? and t1.state=\"2\" and t2.type=\"F\" and "+
		"t1.document not in (select d.documentfrom from docdependencies d where d.documentto = ? and d.type in (\"S\", \"P\") )";
	public static final String findAssortimentByStorageProductAfterDateWhereClause = "t1.from = ? and exists "+
		"(select i.* from i13nactresourceset i where i.document = t1.document and i.resource = ? )"+
		"and t1.blankdate >= ? and t1.state=\"2\" and t2.type=\"P\" and "+
		"t1.document not in (select d.documentfrom from docdependencies d where d.documentto = ? and d.type in (\"S\", \"P\") )";
	public static final String findAgregateByStorageSerialAfterDateWhereClause = "exists"+
		"(select s.* from storagecards s where s.storagecard = t2.agregate and s.serialnumber=?)"+
		"and t1.blankdate >= ? and t1.state=\"2\" and t2.type=\"A\" and "+
		"t1.document not in (select d.documentfrom from docdependencies d where d.documentto = ? and d.type in (\"S\", \"P\") )";
/**
...
public java.sql.PreparedStatement findI13nActByQBE( Boolean isDate, java.sql.Date argDateFrom, java.sql.Date argDateTo, 
	...
	Integer argStorage, Boolean isStorageType, String storageType, Integer order ) throws Exception;
}
