...
/**
 * EJSJDBCPersisterCMPPowerSupplySetBean_caaa5cc8
 */
public class EJSJDBCPersisterCMPPowerSupplySetBean_caaa5cc8 extends EJSJDBCPersister implements com.hps.july.persistence.EJSFinderPowerSupplySetBean {
	private static final String[] _createString = {"INSERT INTO resourcesets (resourceset, mainpartqty, type, name, resourcesetclass, nfsitem_id, mainpart) VALUES (?, ?, ?, ?, ?, ?, ?)", "INSERT INTO powersupplyset (resourceset, power) VALUES (?, ?)"};
	private static final String[] _removeString = {"DELETE FROM powersupplyset  WHERE resourceset = ?", "DELETE FROM resourcesets  WHERE resourceset = ?"};
	private static final String[] _storeString = {"UPDATE powersupplyset  SET power = ? WHERE resourceset = ?", "UPDATE resourcesets  SET mainpartqty = ?, type = ?, name = ?, resourcesetclass = ?, nfsitem_id = ?, mainpart = ? WHERE resourceset = ?"};
	private static final String _loadString = "SELECT T2.mainpartqty, T2.resourceset, T2.type, T2.name, T2.resourcesetclass, T2.nfsitem_id, T2.mainpart, T1.power FROM powersupplyset  T1, resourcesets  T2 WHERE T2.resourcesetclass = 1 AND T2.resourceset = T1.resourceset AND T2.resourceset = ?";
	private static final String _loadForUpdateString = _loadString + " FOR UPDATE";
	private byte[] serObj = null;
	/**
	 * EJSJDBCPersisterCMPPowerSupplySetBean_caaa5cc8
	 */
	public EJSJDBCPersisterCMPPowerSupplySetBean_caaa5cc8() throws java.rmi.RemoteException {
		super();	}
	/**
	 * postInit
	 */
	...
	public void _create(EntityBean eb) throws Exception {
		...
		PowerSupplySetBean b = (PowerSupplySetBean) eb;
...
return null;
	 ...
	 * findByPrimaryKey
	 */
	public com.hps.july.persistence.PowerSupplySet findByPrimaryKey(com.hps.july.persistence.ResourceSetKey primaryKey) throws java.rmi.RemoteException, javax.ejb.FinderException {
		ResultSet resultSet = null;
		PreparedStatement pstmt = null;
		com.hps.july.persistence.PowerSupplySet result = null;

		EJSJDBCFinder tmpFinder = null;
		com.hps.july.persistence.ResourceSetKey _primaryKey = (com.hps.july.persistence.ResourceSetKey)primaryKey;
		try {
			try {
				result = (com.hps.july.persistence.PowerSupplySet)home.getBean(primaryKey);
			}
			catch (Throwable ex) {
				result=null;
			}
			if (result == null) {
				...
				if (tmpFinder.hasMoreElements()) {
					result = (com.hps.july.persistence.PowerSupplySet)tmpFinder.nextElement();
				}
			}
		}
		catch (Exception ex) {
