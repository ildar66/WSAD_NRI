<%@ page language = "java" %>
<%@ page import="org.apache.struts.taglib.html.*,org.apache.struts.util.*,com.hps.july.web.util.EditForm" %>
...
if(map.equals("1")){
	...
	EditForm wrapForm = (EditForm)pageContext.getAttribute(Constants.BEAN_KEY, PageContext.REQUEST_SCOPE);
		if (wrapForm != null) {
			if("View".equals(valueAction)){
				out.print("<A HREF='" + "/siteinfo/PositionListMap.do?mapMenu=0" + "' TITLE=\"Отказ и возврат\"><IMG SRC=\"/images/cancel.gif\" ALT=\"Отказ и возврат\" BORDER=0></A>");
			} else {
	...
	EditForm wrapForm = (EditForm)pageContext.getAttribute(Constants.BEAN_KEY, PageContext.REQUEST_SCOPE);
		if (wrapForm != null) {
			if("View".equals(valueAction)){
				out.print("<A HREF='" + "/siteinfo/PositionOnmap.do?positionMenu=0" + "' TITLE=\"Отказ и возврат\"><IMG SRC=\"/images/cancel.gif\" ALT=\"Отказ и возврат\" BORDER=0></A>");
			} else {
				out.print("<A HREF='" + "/siteinfo/EditPosition.do?action=View" + "' TITLE=\"Отказ и возврат\"><IMG SRC=\"/images/cancel.gif\" ALT=\"Отказ и возврат\" BORDER=0></A>");
			}
		} else {
