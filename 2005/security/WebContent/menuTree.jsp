...
<%
	...
	boolean isArendaIG = isArendaMainEconomist || isArendaEconomist || isArendaDirector;
	boolean isArendaRG = isArendaAccountant || isArendaPurchaser;
	boolean isArendaIRG = isArendaIG && isArendaRG;
	//группы:
...
--%>
	...
	if (isArendaGroup && curReg != null) { 	%>
		...
		<% if (isArendaIG || isArendaRG || isPermitGroup) { %>		
				item1_6_12 = addItem(item1_6, '', 'Описи документов', '');
		<% 	if ( (isArendaIG && !isArendaIRG) || isPermitGroup) { %>
		...
		<% 	}  %>
		<% 	if ( (isArendaRG && !isArendaIRG) || isPermitGroup) { %>
