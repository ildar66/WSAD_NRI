...
<%
	...
	BanksListForm listForm = (BanksListForm)session.getAttribute("BanksListForm");
...
<july:browseform action="/BanksList.do" styleId="listForm">
...
<% } %>
    ...
    <logic:iterate id="org" name="browseList">
		...
		%> 	
		<%if(!idDelete){%>       
            <tr class="normal"  id='<bean:write name="org" property="idBank" filter="true"/>' onMouseOver="className='select'" onMouseOut="onMouseOutRow(this.id, 'normal');" onclick='onClickRow(this.id);' style="font-size:8pt;">
        <%}else{%>
        	<tr class="gray"  id='<bean:write name="org" property="idBank" filter="true"/>' onMouseOver="className='select'" onMouseOut="onMouseOutRow(this.id, 'gray');" style="font-size:8pt;">
        <%}%>
                <td class="text">
	                <%if(!idDelete){%>
