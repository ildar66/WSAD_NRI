<!-- <A href="javascript:Go('idVendor|vendorCode|name|inn|name|legalAddress|accountManagerFio|directorFio|directorJob|typeVendor -->
<%@ page language = "java" %>
...
<%
	...
	VendorLookupListForm listForm = (VendorLookupListForm)session.getAttribute("VendorLookupListForm");
...
<july:browseform action="/VendorLookupList.do" styleId="listForm">
    ...
    <TR>
        ...
        <td class="title">
			<july:select style="font-size:9pt;" styleClass="seach" onchange="switchFilter()" property="filter" insertable="true" updatable="true">
				...
				<html:option value="2"><bean:write name="VendorLookupListForm" property="vendorFormView.nameInn"/> поставщика</html:option>
				<html:option value="3">коду поставщика</html:option>
				<html:option value="4">наименование отделения</html:option>
		   	</july:select>
		</td>
		<td class="text" nowrap>
			<july:string style="width: 200; font-size:9pt;" property="searchString" styleId="searchString" styleClass="search" size="25" insertable="true" updatable="true"/>
		</td>
		<td class="text" nowrap>
			<july:searchbutton/>
		    Тип: <july:select style="width: 150; font-size: 9pt;" property="typeVendor" insertable="true" updatable="true" styleId="typeVendor" styleClass="seach">
...
<% if(isEdit) { %>              
				<A href='VendorDialog.do?action=Add' onclick='return openDialog(this.href, "VendorDialog", "top=300, left=300, width=600, height=350, scrollbars=yes, resizable=yes");'>
					<IMG SRC="/images/plus.gif" BORDER='0' ALT='Добавить поставщика'></A>              	
<% } %>					
              	<%--july:backbutton/--%>
              </TD>
              <TD><B style="font-size: 10pt; color: red">Поставщики:</B></TD>              
    ...
    <tr class="title">
		...
		<TD class="title">
			<A title="Сортировать по полю '<bean:write name="VendorLookupListForm" property="vendorFormView.nameInn"/>'" href="javascript:SortBy('inn')"><bean:write name="VendorLookupListForm" property="vendorFormView.nameInn"/> поставщика</A>
			<%=isUp(sortBy, "inn")%>
		</TD>
		<%--TD class="title">&nbsp;</TD--%>
<% if(isEdit) { %>
...
<% } %>
    ...
    <logic:iterate id="org" name="browseList">
		...
		<%
			...
			String vendorName = vendorVO.getName();
        ...
        <%}%>
                <td class="text">
	                <%if(!(idDelete || noUseNri)){%>
						<A href='VendorDialog.do?action=<%=editAction%>&id=<bean:write name="org" property="idVendor" filter="true"/>' onclick='return openDialog(this.href, "VendorDialog", "top=300, left=300, width=600, height=350, scrollbars=yes, resizable=yes");'><IMG SRC="/images/izmena.gif" BORDER='0' ALT='Изменить поставщика'></A>
					<%}else{%>
						<A href='VendorDialog.do?action=View&id=<bean:write name="org" property="idVendor" filter="true"/>' onclick='return openDialog(this.href, "VendorDialog", "top=300, left=300, width=600, height=350, scrollbars=yes, resizable=yes");'><IMG SRC="/images/izmena.gif" BORDER='0' ALT='Просмотр запись'></A>					
					<%}%>
				</td>
				<td class="text">
                	<%if(!(idDelete || noUseNri)){%>
                		<%if(isChange){%>
                			<A href="javascript:Go('<bean:write name="org" property="idVendor" filter="true"/>|<bean:write name="org" property="vendorCode" filter="true"/>|<%= vendorName%>|<bean:write name="org" property="inn" filter="true"/>|<%= vendorName%>|<bean:write name="org" property="legalAddress" filter="true"/>|<bean:write name="org" property="accountManagerFio" filter="true"/>|<bean:write name="org" property="directorFio" filter="true"/>|<bean:write name="org" property="directorJob" filter="true"/>|<bean:write name="org" property="typeVendor" filter="true"/>')" title="Выберите поставщика">выбрать</A>
                		<%}else{%>
                			<A href="javascript:Go('<bean:write name="org" property="idVendor" filter="true"/>|<bean:write name="org" property="vendorCode" filter="true"/>|<bean:write name="org" property="name" filter="true"/>|<bean:write name="org" property="inn" filter="true"/>|<bean:write name="org" property="name" filter="true"/>|<bean:write name="org" property="legalAddress" filter="true"/>|<bean:write name="org" property="accountManagerFio" filter="true"/>|<bean:write name="org" property="directorFio" filter="true"/>|<bean:write name="org" property="directorJob" filter="true"/>|<bean:write name="org" property="typeVendor" filter="true"/>')" title="Выберите поставщика">выбрать</A>
                		<%}%>
                    <%}else{%>
                    		&nbsp;
                    <%}%>				
				...
				<%--td class="link">
                    <A href='VendorSiteList.do?idVendor=<bean:write name="org" property="idVendor" filter="true"/>'>отд.</A>
                </td--%>                
<% if(isEdit) { %>
				<td class="link">
					<%if( !idDelete && isEdit && (vendorVO.getSource().intValue() == 2) ){%>
                    	<A href='VendorDialog.do?action=Delete&id=<bean:write name="org" property="idVendor" filter="true"/>' onclick='return openDialog(this.href, "VendorDialog", "top=300, left=300, width=600, height=350, scrollbars=yes, resizable=yes");'><IMG SRC="/images/del.gif" BORDER='0' ALT='Удалить поставщика'></A>
                    <%}%>
                </td>
<% } %>
            </tr>
