...
<%
	...
	VendorSiteAdminListForm listForm = (VendorSiteAdminListForm)session.getAttribute("VendorSiteAdminListForm");
	String sortBy = listForm.getSortBy();
	boolean isAdminEdit = request.isUserInRole("administrator") || request.isUserInRole("developer");
	boolean isEdit = isAdminEdit || request.isUserInRole("ArendaMainEconomist") || request.isUserInRole("ArendaEconomist");
%>

<july:browseform action="/VendorSiteAdminList.do" styleId="listForm">
...
<TR class="normal">
 <TD COLSPAN="<%= gColSpan %>">
   ...
   <table class="find" style="font-size:9pt;">
    <TR>
        <td class="title">
         	<I style="font-size: 8pt;">Наименование: </I><bean:write name="VendorSiteAdminListForm" property="vendor_VO.name" />
        </td>
        <td class="title">
        	<I style="font-size: 8pt;">инн: </I><bean:write name="VendorSiteAdminListForm" property="vendor_VO.inn" />
        </td>
        <td class="title"><july:searchbutton/><td>
        <%-- if(isAdminEdit) { %>
        <td class="title" nowrap>
        	по текущему региону: <july:checkbox property="isRegionID" insertable="true" updatable="true" />
        </td>        
        <% } --%>
	</TR>   
<%--   
      ...
      <tr class="title">
          ...
          <TABLE class="buttons">
            <TR class="title">
              <TD class="buttons">
<% if(isEdit) { %>              
				<A href='VendorSiteAdminDialog.do?action=Add&idVendor=<bean:write name="VendorSiteAdminListForm" property="idVendor" filter="true"/>' onclick='return openDialog(this.href, "VendorSiteAdminDialog", "top=300, left=300, width=600, height=350, scrollbars=yes, resizable=yes");'>
...
<% } %>					
    ...
    <tr class="title">
        <TD class="title">&nbsp;</TD>
		<%--TD class="title">
			<A title="Сортировать по полю 'idVendorSiteAdmin'" href="javascript:SortBy('idVendorSite')"><bean:message key="label.code"/></A>
    ...
    <logic:iterate id="org" name="browseList">
        ...
        <%}%>
                <td class="text">
	                <%if(!(idDelete || noUseNri)){%>
	                	<% if(isAdminEdit){%>
							<A href='VendorSiteAdminDialog.do?action=<%=editAction%>&id=<bean:write name="org" property="vo.idVendorSite" filter="true"/>' onclick='return openDialog(this.href, "VendorSiteAdminDialog", "top=300, left=300, width=600, height=350, scrollbars=yes, resizable=yes");'><IMG SRC="/images/izmena.gif" BORDER='0' ALT='Изменить запись'></A>
						<% } else {%>
							<A href='VendorSiteAdminDialog.do?action=<%=editAction%>&id=<bean:write name="org" property="vo.idVendorSite" filter="true"/>' onclick='return openDialog(this.href, "VendorSiteAdminDialog", "top=300, left=300, width=600, height=350, scrollbars=yes, resizable=yes");'><IMG SRC="/images/izmena.gif" BORDER='0' ALT='Изменить запись'></A>
						<% } %>
					<%}else{%>
						<A href='VendorSiteAdminDialog.do?action=View&id=<bean:write name="org" property="vo.idVendorSite" filter="true"/>' onclick='return openDialog(this.href, "VendorSiteAdminDialog", "top=300, left=300, width=600, height=350, scrollbars=yes, resizable=yes");'><IMG SRC="/images/izmena.gif" BORDER='0' ALT='Просмотр записи'></A>
					<%}%>
				</td>
                <%--td class="number">
...
<% if(isEdit) { %>
				<td class="link">
					<%if( !idDelete && isEdit && (siteVO.getSource().intValue() == 2) ){%>
                    	<A href='VendorSiteAdminDialog.do?action=Delete&id=<bean:write name="org" property="vo.idVendorSite" filter="true"/>' onclick='return openDialog(this.href, "VendorSiteAdminDialog", "top=300, left=300, width=600, height=350, scrollbars=yes, resizable=yes");'><IMG SRC="/images/del.gif" BORDER='0' ALT='Удалить запись'></A>
                    <%}%>
                </td>
<% } %>
            </tr>
