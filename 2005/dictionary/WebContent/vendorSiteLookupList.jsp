<!-- возвращает:  Go( idVendorSite|vendorSiteCode|name|kpp|address|vendor_VO.inn||||||vendor_VO.vendorCode|vendor_VO.name|idVendorSiteNfs ) -->
<%@ page language = "java" %>
...
<%--    
    ...
    <TR>
        <td class="title">
         	<I style="font-size: 8pt;">Наименование: </I><bean:write name="VendorSiteLookupListForm" property="vendor_VO.name" />
        </td>
        <td class="title">
        	<I style="font-size: 8pt;">инн: </I><bean:write name="VendorSiteLookupListForm" property="vendor_VO.inn" />
        </td>
	</TR>
	</logic:present>   
--%>  
    <TR>
        ...
        <td class="title">
			<july:select styleClass="seach" onchange="switchFilter()" property="filter" insertable="true" updatable="true">
				...
				<html:option value="1">наименование отделения</html:option>
				<html:option value="2">КПП отделения</html:option>
				<html:option value="3">наименование поставщика</html:option>
				<html:option value="4"><bean:write name="VendorSiteLookupListForm" property="vendorFormView.nameInn"/> поставщика</html:option>
				<html:option value="5">коду поставщика</html:option>				
		   	</july:select>
		</td>
...
<% if(isEdit) { %>              
				<A href='VendorSiteDialog.do?action=Add&idVendor=<bean:write name="VendorSiteLookupListForm" property="idVendor" filter="true"/>' onclick='return openDialog(this.href, "VendorSiteDialog", "top=300, left=300, width=600, height=350, scrollbars=yes, resizable=yes");'>
					<IMG SRC="/images/plus.gif" BORDER='0' ALT='Добавить отделение'></A>              	
<% } %>					
              	<%--july:backbutton/--%>
              </TD>
              <TD><B style="font-size: 10pt; color: red">Отделения:</B></TD>
    ...
    <tr class="title">
		...
		<TD class="title">
			<A title="Сортировать по полю 'v.vendorCode'" href="javascript:SortBy('v.vendorCode')">Код пост.</A>
			<%=isUp(sortBy, "v.vendorCode")%>
		</TD>		
		<TD class="title">
			<A title="Сортировать по полю 'v.name'" href="javascript:SortBy('v.name')">Поставщик</A>
			<%=isUp(sortBy, "v.name")%>
		</TD>		
		<TD class="title">
			<A title="Сортировать по полю 'vs.name'" href="javascript:SortBy('vs.name')">Отделение</A>
			<%=isUp(sortBy, "vs.name")%>
		</TD>
		<TD class="title">
...
<% } %>
    ...
    <logic:iterate id="org" name="browseList">
        ...
        <%}%>
                <td class="text">
	                <%if(!(idDelete || noUseNri)){%>
						<A href='VendorSiteDialog.do?action=<%=editAction%>&id=<bean:write name="org" property="vo.idVendorSite" filter="true"/>' onclick='return openDialog(this.href, "VendorSiteDialog", "top=300, left=300, width=600, height=350, scrollbars=yes, resizable=yes");'><IMG SRC="/images/izmena.gif" BORDER='0' ALT='Изменить отделение'></A>
					<%}else{%>
				...
				<td class="link">
                    <A href='AccountsLookupList.do?idVendorSite=<bean:write name="org" property="vo.idVendorSite" filter="true"/>' onclick="return openDialog(this.href, 'AccountsLookupList', 'top=250, left=300, width=600, height=300, scrollbars=yes, resizable=yes');">р.счета</A>
                </td>								
                <%--td class="number">
...
<% if(isEdit) { %>
				<td class="link">
					<%if( !idDelete && isEdit && (siteVO.getSource().intValue() == 2) ){%>
                    	<A href='VendorSiteDialog.do?action=Delete&id=<bean:write name="org" property="vo.idVendorSite" filter="true"/>' onclick='return openDialog(this.href, "VendorSiteDialog", "top=300, left=300, width=600, height=350, scrollbars=yes, resizable=yes");'><IMG SRC="/images/del.gif" BORDER='0' ALT='Удалить отделение'></A>
                    <%}%>
                </td>
<% } %>
            </tr>
