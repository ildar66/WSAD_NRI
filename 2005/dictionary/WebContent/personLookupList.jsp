...
<%
	...
	PersonLookupListForm listForm = (PersonLookupListForm)session.getAttribute("PersonLookupListForm");
	String sortBy = listForm.getSortBy();
%>

<july:browseform action="/PersonLookupList.do" styleId="listForm">
...
<TR class="normal">
 <TD colspan="<%= gColSpan %>">
    ...
    <table class="find">
     <TR>
       <td class="title" nowrap>по ФИО:</td>
		<td class="title" nowrap>
			<july:checkbox property="isName"  onclick="switchName(document.all.isName[0].checked)" insertable="true" updatable="true" />
        ...
        <logic:iterate id="person" name="browseList" type="com.hps.july.dictionary.valueobject.Person_VO">
            ...
            <tr class="normal" id='<bean:write name="person" property="organization" filter="true"/>' onMouseOver="className='select'" onMouseOut="onMouseOutRow(this.id);" style="font-size:8pt;" onclick='onClickRow(this.id);'>
                ...
                <td class="text" align="right">
					<A href='javascript:Go("<bean:write name="person" property="organization" filter="true"/>|<bean:write name="person" property="name" filter="true"/>")' title="Выберите запись">
						<bean:write name="person" property="name" filter="true"/></A>
                </td>
                <td class="text">
