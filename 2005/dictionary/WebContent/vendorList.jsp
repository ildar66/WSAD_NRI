...
<%
	...
	VendorListForm listForm = (VendorListForm)session.getAttribute("VendorListForm");
...
<july:browseform action="/VendorList.do" styleId="listForm">
    ...
    <TR>
        ...
        <td class="title">
			<july:select style="font-size: 9pt;" styleClass="seach" onchange="switchFilter()" property="filter" insertable="true" updatable="true">
				...
				<html:option value="2"><bean:write name="VendorListForm" property="vendorFormView.nameInn"/> поставщика</html:option>
...
<% } %>					
    ...
    <tr class="title">
		...
		<TD class="title">
			<A title="Сортировать по полю '<bean:write name="VendorListForm" property="vendorFormView.nameInn"/>'" href="javascript:SortBy('inn')"><bean:write name="VendorListForm" property="vendorFormView.nameInn"/> поставщика</A>
			<%=isUp(sortBy, "inn")%>
		</TD>
		<TD class="title">&nbsp;</TD>
<% if(isEdit) { %>
...
<% } %>
    ...
    <logic:iterate id="org" name="browseList">
        ...
        <%}%>
                <td class="text">
	                <%if(!(idDelete || noUseNri)){%>
						<A href='VendorDialog.do?action=<%=editAction%>&id=<bean:write name="org" property="idVendor" filter="true"/>' onclick='return openDialog(this.href, "VendorDialog", "top=300, left=300, width=600, height=350, scrollbars=yes, resizable=yes");'><IMG SRC="/images/izmena.gif" BORDER='0' ALT='Изменить запись'></A>
					<%}else{%>
...
<% if(isEdit) { %>
				<td class="link">
					<%if( !idDelete && isEdit && (vendorVO.getSource().intValue() == 2) ){%>
                    	<A href='VendorDialog.do?action=Delete&id=<bean:write name="org" property="idVendor" filter="true"/>' onclick='return openDialog(this.href, "VendorDialog", "top=300, left=300, width=600, height=350, scrollbars=yes, resizable=yes");'><IMG SRC="/images/del.gif" BORDER='0' ALT='Удалить запись'></A>
                    <%}%>
                </td>
<% } %>
            </tr>
